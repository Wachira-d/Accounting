...
| **5** รายงานรายสาขา | ✅ | JE ของ POS ติดมิติสาขา (เฟส 1) · `GetBranchSummaryAsync(from,to)` + `GET pos/reports/branches` · แท็บ "รายสาขา" ในหน้ารายงาน POS พร้อม**คำเตือนเมื่อมีบิลที่ยังไม่ผูกสาขา** (ไม่ซ่อน — ไม่งั้นผลรวมรายสาขาไม่เท่ายอดบริษัทโดยไม่มีใครรู้ว่าทำไม) |
| **6** ราคา + สิทธิ์ | 🔨 ครึ่งหลัง | `CompanyUser.AllowedBranchIds` + `Helpers/BranchScope` + ด่านตอนเปิดกะ (**ค่าว่าง = ทุกสาขา** เพื่อไม่ล็อก tenant เดิมออกจากระบบ) — เหลือ `PriceList` ราคาต่างตามสาขา |
