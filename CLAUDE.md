...
- **entity ที่ "ไม่มี global query filter" คือที่ที่ soft-delete ไม่มีผลจริง** เรพนี้ตั้ง `HasQueryFilter(e => !e.IsDeleted)`
  ไว้ 203 จุด จนทุกคนอ่านโค้ดแล้วสันนิษฐานว่า "ตั้ง `IsDeleted = true` แล้วแถวหายจากทุก query" — แต่ `SitePage`
  **ไม่มี** ⇒ หน้าที่ลูกค้าลบไปแล้วยัง **เปิดจาก URL ได้ต่อไป** (หายแค่จากเมนู เพราะ query ของเมนูบังเอิญกรองเอง),
  ยังโชว์ในรายการหน้าของตัวแก้ไข และยังถูกนับในโควตา
  → ก่อนเขียน/แก้เส้นที่พึ่ง soft-delete ให้ `grep` ว่า entity นั้นมี `HasQueryFilter` จริงไหม — ถ้าไม่มี
  ต้องกรอง `!IsDeleted` **ทุก query ที่ผู้ใช้หรือผู้เยี่ยมชมเห็น** (และอย่าลืมตัวนับโควตา/ตัวตรวจชื่อซ้ำ) ·
  ญาติของกฎ M: "`CompanyId` ต้องใส่เองเมื่อ global filter ช่วยไม่ได้" — ที่นี่คือ `IsDeleted`
