...
**R2-1 (สูง · M4 ทิศใหม่) "ไม่มีแถว TaxReport" ถูกนับว่า "งวดยังไม่ยื่น" ⇒ VAT ของการริบหายจากทุกแบบที่ยื่น**
...
**R2-2 (สูง · M2 เกินขอบเขต) ใบเต็มยอดเดิม (ก่อน 24/09) ถูกตีเป็น "VAT 0 โดยชอบ" ⇒ ริบแล้วรายได้ไม่มี VAT เงียบ ๆ**
...
**R2-3 (กลาง-สูง · M5) "ส่งมอบแล้ว" (ค่าเริ่มต้นหน้าเว็บ) กับเงินประกันเต็มยอด = รายได้ขาย 41000 ไม่มี VAT**
...
**R2-4 (กลาง · C2) ตัดชำระหลายใบแล้ว void ใบมัดจำ ⇒ ลูกหนี้ subledger ≠ GL**
...
**R2-5 (กลาง · C2 regression) purge ใบที่เคย void ⇒ ลบ JV ต้นฉบับทิ้ง ทิ้งตัวกลับกำพร้า + หัก Realized ซ้ำ**
...
**R2-6 (กลาง · C2 ครึ่งทาง) เส้น "หักมัดจำเป็นฐาน" (drives) ยัง one-shot**
...
## PLAUSIBLE
- P-a ปิดไม่ครบ: ล็อก `DepositRealize` มีแค่ปุ่ม · เส้นอนุมัติ (`LoadTaxedDepositsByRefAsync` FOR UPDATE) ไม่ถือคีย์เดียวกัน และ `RealizeDepositAsync` ไม่ล็อกแถว ⇒ อ่านยอดเก่าแล้วเขียนทับ (lost update)
- P1 ค้าง: รับเงินประกันที่พัก (Lifecycle ~1256) และ `UpdateDocumentAsync` จัดรูปซ้ำ (:2269) ไม่ส่ง `depositChannelVatRate` · หน้าศูนย์มัดจำส่ง channel=null ⇒ ใบที่พัก ChargeVat=false ที่ออก 24–25/09 ริบแบบราคาได้ใบกำกับ 7%
- `RevertTrackedChangesSinceAsync` คืน collection ได้ แต่ reference navigation ของ entity เดิมที่ชี้ entity ใหม่ไม่ถูกตัด ⇒ `SaveChanges` ใน FailLoud อาจดึงกลับเป็น Added (ไม่พบจุดเกิดจริง)
- JobLock: ถ้า connection พังระหว่างงาน `pg_advisory_unlock` ใน finally โยนทับ error เดิม
