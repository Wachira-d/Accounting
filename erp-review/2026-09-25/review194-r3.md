...
✅ `<pending>` **R3-1 (สูง-กลาง) R2-1 ปิดครึ่งเดียว — เส้น "ออกใบกำกับของยอดที่ริบ" ยังให้ GL 21911 กับ ภ.พ.30 คนละเดือน**
...
✅ `<pending>` **R3-2 (กลาง) P-a "ทุกเส้นถือคีย์เดียวกัน" (ข้อ 8 ในคอมมิต) ไม่จริง — เส้นที่อ่าน-เขียนสถานะใบมัดจำโดยไม่ถือ `DepositRealizeKey`**
...
## PLAUSIBLE
- ✅ `<pending>` **P-1 ล้มเพราะ "รอสักครู่" ตอนอนุมัติ**: ฝั่ง DB rollback สะอาด (tx :5855/:6294-6298 · เลขเอกสารใช้ max+xact lock ⇒ ไม่มีเลขขาด) แต่ entity ใน context ยังถูกแก้ค้าง (Status/เลข) ⇒ ผู้เรียกที่ catch แล้ว `SaveChanges` ต่อ (`CmsBookingService.cs:614-623`) จะได้ใบ "Approved ไม่มี JE" (defect class นี้มีอยู่ก่อน แต่ครั้งนี้เพิ่มสาเหตุชั่วคราวใหม่) · เช็คเอาต์ที่พักล้มก่อนประทับ `FinalDocumentId` (`Lifecycle.cs:737`) ⇒ กดซ้ำแล้วใบร่างเดิมค้างอยู่
- ✅ `<pending>` **P-2 บอกว่าไม่ว่างทั้งที่ว่าง**: ล็อกทุก id ที่เลขอ้างอิงตรง**ก่อน** `ResolveDeductedDeposits` ⇒ เลขอ้างอิงเดียวกันของมัดจำค่าห้องกับเงินประกัน ถ้ากำลังคืนเงินประกันอยู่ การอนุมัติเช็คเอาต์จะล้ม (ผู้ใช้เห็น และกดใหม่ได้)
...
- ✅ `<pending>` **P-4**: `UnrealizeDrivesDepositAsync` เทียบ `DocumentNumber == DepositAppliedRef` (:8685-8690) ⇒ เลขอ้างอิงหลายใบไม่เคยตรง ⇒ void แล้วยอดคงเหลือในบัญชีย่อยมัดจำไม่คืน (บั๊กมีอยู่ก่อน แต่ R2-6 เปิดเส้นนี้ให้ใบที่ริบบางส่วน)
...
## สถานะการแก้ (ทีม M3 · รอบ 194)
ยืนยันเองทุกข้อที่ติ๊ก (เปิดไฟล์ตรงบรรทัด) แล้วแก้ในคอมมิตเดียว — รายละเอียด DOCUMENT_FLOW "รอบ 194 ทีม M3" · เทสต์ `DepositRound194R3Tests`
- R3-1: `ForfeitTaxPointDecision(DepositForfeitVatRoute …)` แยกสองเส้น · ใบกำกับของการริบ `PaymentDate: null` ⇒ tax point = วันที่ของใบ + ธง LATE-VAT
- R3-2: `LockDepositBalancesAsync` ตัวเดียว — (ก) ปุ่ม 🧾 VAT (ข) เส้นขับ JE หลายใบ+ใบเดียว (ค) void (รวม 2b/2b-R/7c/ใบมัดจำเอง) + purge (ง) `PostCashSaleJournalAsync` เปิดธุรกรรมเอง ·
  ไล่ `callers.py`: 9 เมธอดเขียนยอด/สถานะใบมัดจำ (37 จุดเขียน) — ทุกเมธอดอยู่ใต้คีย์ผ่านจุดถือคีย์ 9 จุด (ปุ่มรับรู้/ริบ session · อนุมัติหักฐาน · ตัดชำระ · คืน · ปุ่ม VAT ·
  ขับ JE หลายใบ · ขับ JE ใบเดียว · void · purge)
- P-1: CMS ถอยด้วย `TrackedChangeRevert` + หมายเหตุบนการจอง · ที่พักเช็คเอาต์ใช้ใบร่างเดิม (`LodgingCheckoutDraft`) · P-2 ล็อกหลังตัดสินใบที่ถูกหัก ·
  P-4 ตัวแยกเลขเดียวกับเส้นหัก + `SplitDrivesUnrealize` + purge ใบที่ void แล้วไม่คืนซ้ำ (คลาสเดียวกับ R2-5 ที่พบระหว่างแก้)
- ไม่แก้รอบนี้ (ตามคำตัดสิน main agent): P-3 (ไม่พบจุดเกิดจริง) · P-5 หมายเหตุ §86 + ธ.ค.→ม.ค. ก่อนวันที่ 15 (ทิศปลอดภัย — backlog) ·
