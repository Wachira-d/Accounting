...
- **รวมสองค่าที่ความหมายต่างกันไว้ในฟิลด์เดียว — ผู้อ่านปลายทางแยกไม่ออก** (รอบ 193 · ฝ่ายค้านรอบสาม R3-1 · ทีม L2 · **แก้แล้ว 04ce362 — ฟิลด์แยก `Document.DepositBaseDeducted`**) —
  เพื่อให้มัดจำที่ออกใบกำกับแล้วหัก "ฐานก่อน VAT" ได้ ตัวแปลง `ConvertTaxedDrivesAsync` บวกฐานมัดจำเข้า `BillDiscountAmount` **ช่องเดียวกับส่วนลดท้ายบิล
  แบบบาทที่ผู้ใช้กรอก** แล้วผู้อ่านสามตัวตีความทั้งก้อนเป็น "ฐานมัดจำ": การรับรู้ตอนอนุมัติ (`BillDiscountAmount − already`) · ป้ายกระดาษ
  ("หักมูลค่ามัดจำตามใบกำกับ …") · ตัวตัดสิน `BillDeductionIsTaxedDeposit` ⇒ ขายเงินสด 7,450 ส่วนลด 100 + มัดจำ 2,000 ใช้เต็ม = อนุมัติล้ม "ฐานไม่พอ
  (ขาด 100)" ชี้ผิดที่ · มัดจำ 10,700 ใช้บางส่วน = รับรู้เกิน 100 **เงียบ** (ลูกค้าได้คืนขาด 107 · รายได้/ภ.พ.30 เกิน) · ส่วนลดการค้าหายจากกระดาษ
  → กติกาเดิมกฎ #4 E "ห้าม reuse field ผิดความหมาย" ในทรงใหม่: ไม่ใช่ "ยืมช่องเก่าเก็บค่าใหม่" แต่ "**บวก**ค่าใหม่เข้าช่องที่มีค่าเดิมของผู้ใช้อยู่แล้ว" —
  ตัวตัดสินที่อ่านช่องนั้นต้องรู้ว่ามีกี่ความหมายปนอยู่ · ทางแก้ที่เสนอ: ฟิลด์ฐานมัดจำแยก (migration + renderer ×2) หรือปฏิเสธส่วนลดบาทร่วมกับมัดจำที่
  ออกใบกำกับแล้ว (เหมือนกรณี %) → **เลือกฟิลด์แยก**: เฉลี่ยลงบรรทัดรวมกันครั้งเดียว (`AllocateBillDeductions`) แล้ว**แยกกลับสองช่อง**
  (`SplitBillDeduction` — แยกไม่ลงตัว = ล้มดัง) · ผู้อ่านทุกตัวอ่านช่องของความหมายตัวเอง · ยอดรวมผ่าน `BillDeductionTotal` ตัวเดียว · migration ย้ายค่าเดิม
  ตามความหมายที่ persist ไว้ (อนุมัติแล้ว = ยอดที่รับรู้จริง · ร่าง = ทั้งก้อน) · เทสต์สองทิศ (`TaxedDepositDeductionTests`): มีส่วนลด → รับรู้เท่าฐานมัดจำเท่านั้น ·
  ไม่มีส่วนลด → 1,869.16 เท่าเดิม · ผู้อ่าน `BillDiscountAmount` ทั้งเรพ 14 ไฟล์ถูกไล่ทีละจุด (POS/CMS/OCR/recurring ใช้ความหมายส่วนลดการค้าอยู่แล้ว)
  _(ญาติที่เจอในรอบเดียวกัน: `ApplyDepositToInvoiceAsync` ตั้ง `DepositAppliedAmount > 0` บนใบที่หักฐานมัดจำอีกแบบไว้แล้ว ⇒ ตัวตัดสินป้ายกลายเป็น false ⇒
  พิมพ์ฐานมัดจำเป็นส่วนลดธรรมดา (B5) — ธงที่ derive จาก "ช่องไหนมีค่า" เปราะเมื่อสองเส้นเขียนช่องเดียวกัน)_

- **VAT ของการคืน/ริบแบบหลายงวดต้องคิดจากยอดสะสม ไม่ใช่ปัดทีละงวด** (รอบ 193 · ฝ่ายค้านรอบสอง N4 · ทีม L2) — `RefundDepositAsync` แยก VAT
  ของแต่ละงวดด้วย `round(ยอดงวดนี้ × VAT/รวม)` ⇒ คืนสองงวดแล้วฐานค้าง 0.01 และงวดสุดท้ายถูกด่าน "คืนเกินฐาน" ปฏิเสธ (จำลอง 20,000 กรณี ล้ม 9.6%) ·
  แนวทาง "ให้งวดสุดท้ายรับเศษ" ถูกทิ้งหลังจำลอง (ตัวบอกว่าเป็นงวดปิดยอดคลาดเอง)
  → สูตร `DepositReversalMath.RefundSplit`: vat = round((คืนแล้ว+ยอดนี้)×VAT/รวม) − round(คืนแล้ว×VAT/รวม) ⇒ ขึ้นกับยอดสะสมอย่างเดียว · คืนครั้งเดียวจากศูนย์ =
  สูตรเดิมทุกประการ · ตัวเดียวทั้ง `RefundDepositAsync` และตัววางแผนที่พัก (ทิศเดียวกับ `PosCogsBooking.RefundCogs` ที่ปัดสะสมระดับบิล)
