...
- **ลงตัวทางคณิตไม่พอ ต้องถามว่า VAT ถูกคิดจากยอดไหน** (รอบ 192) — ใบ Shopee ยอดใบกำกับ 536 (VAT 35.07 = 536×7/107) แล้วมี
  "ส่วนลดพิเศษ 98" + "ยอดชำระ 438" ต่อท้าย ⇒ ถ้า engine หยิบ 438 ด่านคณิตข้อ 0 และเคส E ของตัวกระทบยอด**ลงตัวทั้งคู่แต่ผิดเรื่อง**
  (ตีส่วนลดหลังใบกำกับเป็นส่วนลดในใบกำกับ ⇒ ฐาน/VAT ผิด) · ตัวชี้ขาดคือ VAT ที่พิมพ์ ไม่ใช่ตำแหน่งหรือชื่อป้าย
- **ป้าย "TOTAL" ไม่ใช่ยอดรวมทั้งสิ้นเสมอ — นับชั้นหลักฐานอิสระ และบังคับอธิบายค่าที่แพ้** (รอบ 192) — Makro พิมพ์ "TOTAL 24,110.00"
  เป็นยอด**ก่อน**ส่วนลด ขณะที่ยอดจริง 23,812.25 มีพยาน 4–5 ชั้น (AMOUNT · NET AMOUNT · ตัวอักษร · แถวชำระ · ตารางรหัส ภ.พ.) · ตัวอ่านเดิม
  "regex แรกที่แมตช์ชนะ" + back-calc 7/107 จากยอดผิด ⇒ **แต่ง VAT 1,577.29 ขึ้นมาเองแทน 1,148.28 ที่พิมพ์อยู่** แล้วไม่มีแท็กบล็อก
- **ฝ่ายค้านรอบ 192 จับ 5 ข้อในด่านใหม่ก่อนถึงผู้ใช้** — ค่าเผื่อ 0.02 บน VAT = ±0.31 บาทบนยอดรวม ⇒ ใบปัดเศษ 0.25 ติด "ขัดแย้ง"
  (บล็อกใบถูกเกือบทุกใบ = ปิดด่านโดยไม่ตั้งใจ) · หลักฐานจากบรรทัดเดียวนับสองชั้น · ฐานที่คำนวณถูกประทับว่า "พิมพ์บนกระดาษ 0.95" ·
  รายการแท็กที่คัดลอกตอนอัปโหลดซ้ำเขียนมือแยกจาก `BlockingTags` (ขาดไป 5 ตัวแล้วโดยไม่มีใครรู้) ⇒ **ด่านใหม่ที่บล็อกต้องผ่าน
  ฝ่ายค้านเสมอ และรายการแท็กที่สองต้อง derive จากรายการแรก ไม่ใช่สำเนามือ**

- **เส้น "หลักฐานอันดับหนึ่ง" ที่ไม่มีเทสต์ไฟล์จริงสักใบ = ไม่เคยพิสูจน์ว่าทำงาน** (รอบ 193 · ทีม O1 · คำตัดสิน #10) — `EtaxPdfXmlExtractor` (XML ที่ฝังใน PDF
  e-Tax = ตัวตั้งที่แข็งที่สุด) มีช่อง 7 จุดที่ทำให้**ทั้งไฟล์ตกไป OCR เงียบ ๆ** กับ PDF จริงของ Shopee (pdfmake/PDFKit): Filespec ตัวแรกไม่ใช่ .xml แล้ว
  `IndexOf(b, −1)` โยน · ชื่อไฟล์ hex UTF-16 · regex ชี้ Filespec ไม่เคยแมตช์ · ตัดสตรีมผิดเมื่อ dict มีคำว่า `octet-stream` · ยอดบรรทัดอยู่ใน
  `NetLineTotalAmount` · BOM · `T03` ถูก map เป็น Receipt (ใบกำกับถูกตีว่าไม่ใช่ใบกำกับ) — ไม่มีอะไรฟ้องเพราะ fallback ไป OCR "ก็ได้ผลอยู่ดี"
  → กติกา: เส้นที่มี fallback เงียบต้องมีเทสต์ที่**ล็อกว่าเส้นหลักถูกใช้** (ไม่ใช่แค่ผลสุดท้ายถูก) ด้วยไฟล์/โครงจริง · fixture XML จริงอยู่
  `erp-review/2026-09-24/fixtures/` · ⚠️ ยังไม่มี PDF จริงของ Shopee เป็น fixture (เทสต์สร้าง PDF จำลองโครง PDFKit)

- **"ยอดชำระจริง ≠ ยอดใบกำกับ" ไม่ใช่ส่วนลดในใบกำกับ — ลงที่ขั้นชำระ ไม่ใช่แก้ยอดเอกสาร** (รอบ 193 · คำตัดสิน #1/#3/#4 · ทีม O1) — ใบ Shopee 536
  (VAT 35.07) จ่ายจริง 438 (ค่าส่ง +37 · คูปอง −135) · ถ้าลดยอดเอกสารตามยอดจ่าย = ฐาน/VAT ผิดจากใบกำกับที่ผู้ขายรายงาน · ถ้าไม่ลงส่วนต่างเลย = หนี้ค้าง 98
  ตลอดไป → ยอดเอกสาร = ใบกำกับ · ส่วนต่าง = บรรทัดปรับตอนชำระ (51120/51150) · สแกนเสนอบรรทัดปรับแต่**ปลดด่านเฉพาะเมื่อข้อเสนอลงตัวกับยอดเอกสาร**
  (`OcrSettlementProposal.FitsDocument` — ฝ่ายค้านรอบสอง N5: รุ่นแรกเขียน `[PAY-AT-PAYMENT]` แม้ไม่มีข้อเสนอ ⇒ แท็กบอกว่า "ถูกต้อง" ทั้งที่ยังไม่ได้ตรวจ)

- **ตัวเดาจากชื่อสินค้าต้องอยู่ "ใต้" หลักฐานตัวเลขทั้งใบ — ไม่ใช่เป็นคำตอบสุดท้ายเมื่อชั้นบนเงียบ** (รอบ 195 · ใบ Scommerce TXE05202609T004679 ·
  ถดถอยจาก `5e3a323b`) — `5e3a323b` ต่อ `ThaiVatTypeRule` เข้าเส้น OCR (ถูกในตัวมันเอง: ใบผสม 7%/ยกเว้น) แต่แทนที่ `headerVat > 0 ? 7 : 0`
  **โดยไม่เหลือชั้นที่ถามกระดาษ** ⇒ "นมผงเอนฟาโกร…" (คำ "นม") = ยกเว้น ทั้งที่กระดาษพิมพ์ 7% × 4,695.33 = 328.67 พอดี + "ยกเว้น 0.00" ⇒ VAT บรรทัด 0 ·
  เอกสาร 4,695.33 ≠ 5,024.00 · คำเตือน 3 ข้อ · ภาษีซื้อ 328.67 หาย · รอบ 192 **ทำนายไว้แล้ว** (`erp-review/2026-09-24/total-B.md` RC-9) แต่ไม่ได้แก้ และเทสต์
  รอบ 193 (`DocumentRoundingTests` ชุด Lazada) สมมติบรรทัด 7% ไว้ก่อนเลยไม่เคยตรวจขั้นตั้งอัตรา → กติกา: (1) ชั้นหลักฐานเรียงตาม "ระยะห่างจากของจริง"
  (DOCTRINE §1) — ตัวเลขที่พิมพ์ทั้งใบ > เดาจากชื่อ · ชั้นใหม่ต้อง "ไม่รู้" ได้ (`OcrLineVatPlanVerdict.Unknown`) แล้วตกไปชั้นเดิม (2) แก้ที่ชั้นหลักฐาน
  ไม่ใช่เติมรายการคำทีละคำ (คำ "นมผง" เพิ่มแล้วก็จริง แต่ชื่อถัดไปที่ขึ้นต้น "นม" จะพังอีก — เทสต์ `ชื่อที่ตัวเดายังตีเป็นยกเว้น_ชั้นพิสูจน์ทั้งใบชนะ`)
  (3) เทสต์ของใบจริงต้องเริ่มจาก**ข้อความ/ยอดที่ engine ให้** ไม่ใช่จากค่ากลางทางที่เราคิดว่าควรเป็น (4) บรรทัดยอด 0 (ค่าส่งฟรี) ไม่ใช่หลักฐาน
  ว่า "มีบรรทัด 7%" และไม่ใช่ที่วาง % ส่วนลด · ข้อความเตือนที่ระบุสาเหตุต้องตรวจสาเหตุนั้น (ส่วนต่างทั้งก้อนมาจาก VAT ≠ "รายการอ่านไม่ได้")
- **สคริปต์จำลองที่อ่านอาร์เรย์คำจากซอร์ส C# ด้วย regex อ่านคอมเมนต์ด้วย** (รอบ 195) — คอมเมนต์ในอาร์เรย์ `NotExemptDespiteKeyword` ที่มี `"นม"` ในเครื่องหมาย
  คำพูดทำให้สคริปต์ก่อน/หลังรายงานว่า "นมสด" ไม่ยกเว้นแล้ว (C# ไม่เห็นคอมเมนต์ — ผลจำลองผิด ไม่ใช่โค้ดผิด) ⇒ คอมเมนต์ในอาร์เรย์คำห้ามมีเครื่องหมายคำพูด
  และผลจำลองที่ขัดกับเทสต์ต้องหาเหตุก่อนเชื่อฝั่งใดฝั่งหนึ่ง
- **ด่านที่ตรวจด้วยสูตรเดียวกับที่ผลิตค่า = ผ่านตลอดกาล** (รอบ 195 ฝ่ายค้าน C1) — `OcrLineVatPlanner` "พิสูจน์" ทั้งใบ 7% ด้วย
  `round(T × 7/107) == VAT` ขณะที่ VAT ตัวนั้นระบบแยก 7/107 เองจากยอดรวม ⇒ ใบผักรวม 1,070 ได้ภาษีซื้อ 70 ที่ไม่มีบนกระดาษ และถอดตาข่าย `[Σ-GAP]` ทิ้ง ·
  ญาติสองตัวในเส้นเดียวกัน: `ValidateOrInferVatRate` ดันความมั่นใจของ VAT ที่ถอดเองเป็น 0.95 เพราะ "= 7% ของฐาน" · `AmountTripleExtractor` fallback แต่ง 7/107
  แล้วถูกรับเป็น "สามค่าที่ลงตัว" ที่มา PaperLabel. กติกา: ก่อนให้ด่านเลขคณิตยืนยันค่า ถาม "**ค่านี้มาจากไหน**" (`OcrHeaderVatEvidence` — อ่านจากข้อความกระดาษ
  ไม่ใช่ความมั่นใจที่ถูกดันด้วยสูตรเดียวกัน) · ตัวผลิตค่าที่คำนวณต้องติดแท็ก (`[VAT back-calc]`) และมีที่เดียว (ด่านเดียว) · เทสต์ทิศตรงข้ามต้องใช้ใบที่
  "ค่าถูกแต่งแล้วเลขคณิตลงตัว" ไม่ใช่แค่ใบที่เลขคณิตไม่ลงตัว
- **ตัวเติมที่รันหลังด่านต้องเคารพคำตัดสินของด่าน** (รอบ 195 C1 ข) — `ParseThaiDocument` ปฏิเสธการแยก VAT แล้ว `SmartFieldExtractor.Enrich` (รันทีหลัง)
  เห็น VAT ว่างจึงเติมทับ · คำปฏิเสธต้องเป็นสัญญาณที่ชั้นถัดไปอ่านได้ (`VatBackCalcGuard.SkipTag`) ไม่ใช่แค่ "ช่องว่าง" ที่ตัวเติมตีความว่า "ยังไม่มีใครทำ"
- **คำปฏิเสธต้องติดคำเป้าหมายและอยู่ในประโยคเดียวกัน** (รอบ 195 C2) — หน้าต่าง 14 ตัวอักษรนับ "ยกเลิกไม่ได้" ของบรรทัดก่อนเป็นคำปฏิเสธหัวใบอย่างย่อ
  ⇒ เคลมภาษีซื้อต้องห้าม · ขอบเขตคำ = บรรทัดเดียวกัน + ติดกัน + มีคำยืนยันความหมาย ("แทน/ฉบับใหม่")

- **ตัวตัดสินที่ "รอเจ้าของตัดสิน" ต้องมีทางออกที่ผู้ใช้เห็น และห้ามเป็นสำเนาที่สองของกติกากลาง** (รอบ 197 · ใบ Makro สาขา 00005) —
  รอบ 190 เขียน `OcrVendorBranchContact` ให้ "ไม่มีแถวสาขาตรง ⇒ ผูก สนญ. ไว้ก่อน (รอเจ้าของตัดสินเรื่องสร้างแถว)" · รอบ 193 ทำคีย์กลาง
  `ContactTaxBranchKey` (แถวว่าง ≡ สนญ. · ไม่พบ ⇒ สร้างแถวสาขา) ให้ทุกทางเข้า **แต่เส้น OCR ไม่ได้ถูกย้ายไปใช้** ⇒ สองกติกาขัดกันในระบบเดียว และผู้ใช้เห็น
  "จับคู่แล้ว" กับผู้ติดต่อ 00000 ทั้งที่ช่องสาขาบนจอเขียน 00005 · บทเรียน: (1) คำถามที่ค้างรอเจ้าของต้องอยู่ในรายการคำถามที่ถูกถามจริง ไม่ใช่ในคอมเมนต์ของ
  enum (2) เมื่อกติกากลางเกิดทีหลัง ให้ `callers.py` หา "ตัวตัดสินรุ่นก่อนที่ทำเรื่องเดียวกัน" แล้วยุบเข้า (ที่นี่แยก `ContactTaxBranchKey.PickBranch` ให้ OCR เรียก)
  (3) เส้นที่ "ใช้ผลที่ persist ไว้" (สร้างเอกสารจาก `MatchedContactId`) ต้องตัดสินซ้ำด้วยตัวเดียวกัน ไม่งั้นสแกนเก่าผิดตลอดไป — โดยเคารพคำตอบที่คนเลือกเอง
- **เลข 13 หลักที่กระดาษแบ่งกลุ่มแปลก = "ไม่มีเลข" ในสายตาตัวหาป้าย แต่ engine ยังส่งเลขมา ⇒ ด่านกุญแจตกโดยไม่มีใครรู้ว่าทำไม** (รอบ 197) —
  Makro พิมพ์ "0 10 7 567 00041 4" · Azure ส่ง `VendorTaxId` ถูก แต่ `ThaiTaxId.Pattern` (1-4-5-2-1) หาไม่เจอ ⇒ `OcrVendorKeyEvidence` Unproven ⇒ ทะเบียนถูกปัด
  ⇒ ชื่อโลโก้ "ma ro" ค้าง พร้อมข้อความ "น่าจะอ่านเลขผู้เสียภาษีผิด" ซึ่ง**ไม่จริง** (หลักการข้อ 7: ข้อความที่ระบุสาเหตุต้องตรวจสาเหตุนั้นจริง) · แก้ด้วย pattern
  หลวมที่รับ**เฉพาะตัวมีป้าย** + ป้ายฉบับ "ต้นฉบับลูกค้า/For Customer" เป็น Noise (ญาติของ "customer copy") · และชื่อที่ไม่มีรูปนิติบุคคลให้ชั้นกระดาษ
  (`OcrVendorLegalName`) ตัดสิน**ก่อน**ตัวเรียนรู้ (ไม่งั้นคลัง known-good จำโลโก้เป็นชื่อที่ถูก)
- **regex "ตัวแรกของหน้า" สำหรับช่องของคู่สัญญาฝั่งเดียว = ได้ของอีกฝั่งเมื่อกระดาษมีบล็อกนั้นก่อน** (รอบ 197) — อีเมลผู้รับสินค้า (เรา) ลงผู้ติดต่อผู้ขาย ·
  ญาติของ "ที่อยู่ผู้ขายที่เป็นบล็อกผู้ซื้อ" (รอบ 190) — ทุกช่องที่เป็น "ของผู้ขาย" (ชื่อ · เลข · ที่อยู่ · เบอร์ · อีเมล) ต้องถามตำแหน่งเทียบป้ายฝั่ง ·
  ป้ายบล็อกผู้รับ/ที่อยู่จัดส่งแยกรายการจากป้ายที่ใช้ตัดสินบทบาท (เพิ่มคำตรงนั้น = เปลี่ยนบทบาทของทุกใบ)
