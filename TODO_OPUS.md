...
### A3. ✅ (เสร็จ) ใบลดหนี้: ช่อง "เลขที่ใบลดหนี้" + แสดงในรายงานภาษี (เฉพาะ CN มี VAT)
...
### A4. ✅ (เสร็จ) ป้าย "ประเภทเอกสาร" ทั้งระบบ แสดงตามหัวกระดาษ
...
**สถานะวันนี้ (คำถามผู้ใช้ "เปิดใบแจ้งหนี้แล้วเห็นง่ายรึยังว่าใบไหน combined"):
ยังไม่ง่ายพอ** — หน้า detail/list แสดงป้ายจากชนิด enum (`docTypeLabel`) เท่านั้น
ใบ combined เห็นได้เฉพาะตอนเปิดแก้ไข (dropdown หัวเอกสาร) หรือตอนพิมพ์ PDF —
งานข้อนี้คือคำตอบ: ป้ายบน **list + หัว detail modal** ต้องบอกหัวจริงทันที
(เช่น badge "ใบแจ้งหนี้/ใบกำกับภาษี" สีต่างจาก "ใบแจ้งหนี้" เปล่า)
...
### A5. ✅ (เสร็จ) ทางรับเงินต้อง "ทางเดียวต่อสถานการณ์" — บันทึกชำระเงิน vs แปลงเป็นใบเสร็จ
...
**ข้อเสนอ (ตัดสินใจแล้ว — ปรับตาม feedback ผู้ใช้: "สองประตู ห้องเดียว"):**
ผู้ใช้คิดสองแบบ — สาย "ฉันรับเงินแล้ว" (เงิน-first) กดบันทึกชำระ, สาย "ฉันจะ
ออกใบเสร็จ" (กระดาษ-first) ไปที่แปลงเอกสาร. **การซ่อนประตูใดประตูหนึ่ง = สายนั้น
ชนกำแพง** ⇒ ทางที่ถูก: **คงทั้งสองทางเข้า แต่บรรจบที่กลไกเดียว**:
1. **เอกสารที่ตั้งหนี้แล้ว (Invoice/TaxInvoice/DebitNote มี AR):** ตัวเลือก
   "ใบเสร็จรับเงิน/ใบสำคัญรับ" ใน convert modal **ยังแสดง** แต่กดแล้ว
   **ไม่เรียก Convert API** — ปิด convert modal แล้ว**เปิด modal บันทึกชำระเงิน
   แทน** (prefill ยอดเต็ม + smart default ติ๊กใบเสร็จตามกติกาเดิม) + toast
   สั้น ๆ "การรับเงิน + ออกใบเสร็จ ทำที่นี่ — ผลเหมือนกันทุกทาง". ได้ครบกว่า
   convert เดิมด้วย (แก้ยอดเป็นบางส่วน/หัก WHT/มัดจำ ได้ในจอเดียว)
   - title ของ option ในรายการแปลง อธิบายไว้เลย: "= บันทึกรับเงิน (เปิดหน้า
     บันทึกชำระให้)" — ผู้ใช้ไม่แปลกใจตอนหน้าเด้ง
   - backend `ValidConversions` คงไว้ (compatibility/API) — เปลี่ยนเฉพาะ
     พฤติกรรม UI; `ValidateConversionAsync` ยัง block เคสผิดเหมือนเดิม
     สำหรับ caller ตรง
   - จุดต่างเดียวที่เสียไป: convert เดิมได้ "ใบเสร็จร่างให้รีวิวก่อนอนุมัติ"
     ส่วนทางนี้ใบเสร็จออกทันทีพร้อมการชำระ — ยอมรับได้ (settlement receipt
     เป็นหลักฐานอัตโนมัติ แก้/พิมพ์ซ้ำได้) — ระบุใน DOCUMENT_FLOW
2. **แหล่ง pre-revenue (Quotation/BillingNote):** convert → Receipt (ขายสด
   รับรู้รายได้เอง) ทำงานแบบ convert จริงเหมือนเดิม — recordPayment ใช้ไม่ได้
   เพราะยังไม่มี AR (สองโลกไม่ทับกัน)
...
4. DOCUMENT_FLOW.md อัปเดตตาราง entry/exit ของ Receipt + TEST_PLAN: เคส
   INV → เลือกใบเสร็จใน convert modal → ได้ modal บันทึกชำระ (ไม่ใช่ Convert
   API) / QT → convert จริงเหมือนเดิม / ข้อความติ๊กตรงหัวจริง / ใบ combined
   จ่ายครบไม่ติ๊ก → พิมพ์ได้หัว 3-in-1

**เกณฑ์ตรวจรับ:** เข้าประตูไหน (บันทึกชำระ / แปลง→ใบเสร็จ) ก็จบที่จอเดียวกัน
ผลบัญชี+เอกสารเหมือนกัน 100% · คำตอบของติ๊กใบเสร็จอ่านครั้งเดียวรู้ว่ามี/ไม่มี
เอกสารใหม่ · QT/BN แปลงเป็นใบเสร็จขายสดได้เหมือนเดิม · ไม่มีเส้นทางที่กดแล้ว
error หรือหายเงียบ

### A6. ✅ (เสร็จ) ไฟล์แนบ/รูปในหน้ารายละเอียดเอกสาร — พับไว้ กดแล้วค่อยแตก

**ปัญหา (feedback ผู้ใช้):** รูป/พรีวิวไฟล์แนบแสดงกางออกในหน้า detail ทันที →
รกและล้นหน้า โดยเฉพาะใบที่มีสแกนหลายไฟล์/ไฟล์ใหญ่

**จุดโค้ด:** `documents.html` `renderAttachmentSection` (~:8658) + ตัวเปิดดู
ในหน้า (~:8848 "เปิดดูไฟล์แนบในหน้าเลย") — ปัจจุบัน preview inline

**งาน:**
1. Default = **แถวรายการอย่างเดียว** (ไอคอนชนิดไฟล์ + ชื่อ + ขนาด + ปุ่ม
   ดู/ดาวน์โหลด/ลบ) — ไม่โหลด blob รูปจนกว่าจะกด (ประหยัดแบนด์วิดท์/หน่วยความจำ
   ด้วย — โค้ดมี `_releaseAttachmentBlobs` เพราะเคยกินเมโมรีอยู่แล้ว)
2. กด "ดู" → แตก inline (รูป = thumbnail ขยายได้ / PDF = iframe) หรือ
   lightbox — กดซ้ำ/ปิดเพื่อพับกลับ; หลายรูปพับ/กางอิสระต่อไฟล์
3. จำนวนไฟล์แสดงบนหัว section ("ไฟล์แนบ (3)") ให้รู้ว่ามีของโดยไม่ต้องกาง
4. เช็คหน้าที่ reuse ตัว render เดียวกัน (OCR review? expense claim?) ให้ได้
   พฤติกรรมเดียวกัน — อย่าแตกเป็นสอง implementation (กฎ "สอง renderer ห้าม drift")

**เกณฑ์ตรวจรับ:** เปิดใบที่มีสแกน 5 รูป หน้า detail สูงเท่าเดิม (ไม่ยืดตามรูป) ·
ไม่มี network โหลดรูปจนกว่าจะกดดู · กด/พับ ทำงานอิสระต่อไฟล์
...
## D. ผล audit OCR → เอกสารทุกประเภท (2026-08-17)

สิ่งที่ **ครบแล้ว** (ยืนยันจากโค้ดจริง): จำแนกชนิดเอกสาร 8 ชนิด + ฝั่งผู้ซื้อ/ผู้ขาย
จากเลขผู้เสียภาษีบนกระดาษ · ฝั่งซื้อ (Expense/PI/PV) เติมครบ: ผู้ขาย/เลขภาษี/สาขา
(สาขาบนใบมาก่อน Contact) · เลขที่+วันที่ใบกำกับผู้ขาย · บรรทัดสินค้า+ส่วนลด ·
VAT (รวม §86/6 บล็อกเคลม + เหตุผล) · WHT + อัตรา · สกุลเงิน · ราคารวม VAT ·
เครดิตเทอม/วันครบกำหนด · ผังบัญชี AI + distillation loop ปิดครบ (กฎเหล็ก #1) ·
CN/DN: เหตุผล §86/10 + ฝั่ง + เลขใบลดหนี้ผู้ขาย (รอบ 65) · confidence <0.85
ไฮไลต์เหลือง · เลข DRAFT ตามกฎ §86/4 · RD re-check ก่อนแสดง

**ช่องว่างที่พบ (backlog ตามลำดับความคุ้ม):**

| # | ช่องว่าง | ผลกระทบ | แนวแก้ |
| --- | --- | --- | --- |
| D1 | ฝั่งขาย (สแกนใบของเราเองเพื่อ re-key): ไม่เติม `BuyerBranchCode` จากกระดาษ — default 00000 | ใบขายให้สาขาลูกค้า ขึ้นรายงานภาษีขายผิดสาขา (ประกาศฯ 199) | อ่าน branch block ฝั่งผู้ซื้อเหมือนที่ทำฝั่งผู้ขายแล้ว (BuyerBranchRegex) |
| D2 | ใบมัดจำสแกนไม่ได้ตรง ๆ — target ไม่มี "Deposit" ต้องเลือก Receipt แล้วติ๊กมัดจำเอง | ผู้ใช้ลืมติ๊ก → ลงรายได้แทนหนี้สินมัดจำ | เพิ่ม pseudo-target Deposit + keyword "มัดจำ/deposit" ใน inference |
| D3 | ใบเสร็จ/ใบกำกับที่สแกนมา **ไม่เช็คซ้ำกับใบที่มีอยู่** ด้วยเลขที่+ผู้ขาย ก่อนสร้าง (dup-detect มีที่ integration แล้ว แต่ OCR path เช็คคนละจุด) | สแกนใบเดิมซ้ำ = ค่าใช้จ่าย/ภาษีซื้อเบิ้ล | เรียก dup-check เดียวกับ integration ตอน handoff + เตือนก่อนสร้าง |
| D4 | หลายหน้า/หลายใบในไฟล์เดียว (batch) — แยกใบอัตโนมัติยังไม่มี | ผู้ใช้ต้องตัดไฟล์เอง | หลังมี usage จริงค่อยทำ (แพง) |
| D5 | ตำแหน่ง AI ที่ยังไม่ได้ใช้: second-opinion โครงสร้าง JE (ต่อจาก JournalPostingGuard รอบ 67) ผ่าน `IAiOrchestrator` + `GenericFeedbackDistillationModel` featureKey ใหม่ | ยกระดับจาก rule → ความสมเหตุสมผลของผังต่อบริบทร้านค้า | ทำตาม checklist กฎเหล็ก #1 ครบ 4 ขั้น (อย่า half-AI) |
