...
public static class AuditHashChain
{
    ...
    /// <item><b>Dangling</b> — PrevHash ชี้ไปยัง hash ที่ไม่มีแถวไหนในบริษัทถืออยู่ = แถวก่อนหน้าหายไป (ถูกลบ) หรือแถวนั้นไม่ได้เกิดจากระบบ</item>
    ...
    public static string? AlertMessage(ChainAnalysis a, int maxIds = 20)
    {
        ...
        if (a.Dangling.Count > 0)
            parts.Add($"ลำดับ audit log ขาดตอน {a.Dangling.Count} จุด — แถวก่อนหน้าของแถวต่อไปนี้หายไป (ถูกลบ) หรือแถวนี้ไม่ได้บันทึกโดยระบบ: {Ids(a.Dangling)}");
