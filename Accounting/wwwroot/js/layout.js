...
const Layout = {
  ...
  // ═══ อัตรา VAT ของบรรทัดเอกสาร: "ใครตั้ง ชนะ ใคร" — ตัวตัดสินตัวเดียวของทั้งระบบ ═══
  //
  // ⚠️ ที่มา (บั๊กจริง ผู้ใช้รายงาน 2026-09-11): บิลเขียนมือของผู้ขายที่ไม่จด VAT
  // (ยอด 3,500 · ช่อง VAT ว่าง · รวม 3,500) สแกนอ่านถูกและฟอร์มเติม 0% ให้ถูกแล้ว —
  // แต่พอผู้ใช้แตะช่อง "รายละเอียด" แล้วคลิกที่อื่น เหตุการณ์ blur เรียก
  // `suggestVatType` ซึ่งถาม endpoint heuristic ว่า "รายการชื่อแบบนี้ปกติคิด VAT
  // เท่าไร" → ตอบ 7% (ความมั่นใจ 0.95) แล้ว **ทับค่าที่อ่านจากกระดาษทันที**
  // ⇒ ยอดสุทธิเด้งจาก 3,500 เป็น 3,745 (VAT 245 ที่ไม่มีอยู่บนกระดาษ)
  // ⇒ ภาษีซื้อที่เคลมไม่ได้จริงถูกนำเข้า ภ.พ.30 และเจ้าหนี้/ยอดจ่ายเกินจริง
  //
  // สองข้อผิดพลาดซ้อนกัน: (1) ตัวเติมแบบ async ที่ "มาทีหลังชนะ" (กฎเหล็ก #4 A)
  // (2) คำตอบ **ทั่วไป** ("รายการแบบนี้ปกติ 7%") ทับ **ข้อเท็จจริงเฉพาะใบ**
  // (กระดาษใบนี้ไม่มี VAT) — ตัวแนะนำไม่เคยเห็นกระดาษเลยสักครั้ง
  //
  // กติกา: ทุกจุดที่เขียนอัตรา VAT ลงบรรทัดต้องผ่าน `Layout.setLineVat` และประกาศ
  // **ที่มา** ของค่า · ที่มาที่อ่อนกว่าเขียนทับที่มาที่แข็งกว่าไม่ได้ (เท่ากันได้ —
  ...
  vatSrcRank(sel) {
    if (!sel) return 0;
    // `userTouched` เป็นธงเดิมที่โค้ดหลายจุดอ่านอยู่แล้ว — ยังถือเป็น "ผู้ใช้เลือกเอง"
    if (sel.dataset.userTouched === '1') return Layout.VAT_SRC_RANK.user;
    return Layout.VAT_SRC_RANK[sel.dataset.vatSrc] || 0;
  },

  /** แหล่ง `src` มีสิทธิ์เขียนทับค่าที่อยู่ในช่องตอนนี้ไหม */
  canSetLineVat(sel, src) {
    return Layout.vatSrcRank(sel) <= (Layout.VAT_SRC_RANK[src] || 0);
  },

  /** ตั้งอัตรา VAT ของบรรทัดพร้อมประทับที่มา — คืน true เมื่อเขียนจริง
   *
   *  ถ้าถูกปฏิเสธเพราะแหล่งที่แข็งกว่าตั้งไว้แล้ว **ต้องไม่เงียบ**: เขียนความเห็นต่าง
   *  ลง tooltip ของช่องให้ผู้ใช้เห็นว่ามีอีกแหล่งเสนอค่าอื่น (กติกา "ไม่ทับก็ต้อง
   *  บันทึกว่ามีความเห็นต่าง") */
  setLineVat(sel, rate, src) {
    if (!sel) return false;
    const val = String(rate);
    if (!Layout.canSetLineVat(sel, src)) {
      if (sel.value !== val) {
        const label = { ai: 'ระบบแนะนำ', doctype: 'ชนิดเอกสาร', ocr: 'ที่อ่านจากกระดาษ',
                        doc: 'ที่บันทึกไว้', product: 'คลังสินค้า', user: 'ที่คุณเลือก' };
        sel.title = `${label[src] || src}เสนออัตรา ${val === '-1' ? 'ยกเว้น' : val + '%'} `
          + `— ไม่เปลี่ยนให้เพราะค่าปัจจุบันมาจากแหล่งที่เชื่อถือได้มากกว่า (แก้เองได้)`;
      }
      return false;
    }
    sel.value = val;
    sel.dataset.vatSrc = src;
    if (src === 'user') sel.dataset.userTouched = '1';
    return true;
  },
  ...
  /** แปลง "ปี" ที่อ่านจากเอกสารไทยเป็น ค.ศ. — **mirror ของ `Helpers/ThaiDate.NormalizeYear`**
   *
   *  ⚠️ ที่มา: หน้าแอดมิน OCR เขียนกติกาของตัวเองว่า "เติม '20' หน้าปีย่อเสมอ แล้ว
   *  ถ้าเกิน currentYear+10 ค่อยลบ 543" ⇒ ปีย่อ พ.ศ. `69` กลายเป็น 2069 → 1526
   *  (**ห่างจริง 500 ปี**) แล้วไป prefill ช่องวันที่ของตัวอย่างฝึกระดับระบบ
   *
   *  ฝั่ง C# ยุบเหลือตัวเดียวไปแล้ว — ฝั่ง JS ต้องมีตัวเดียวเช่นกัน ห้ามหน้าไหน
   *  คิดเกณฑ์เอง (ตารางกฎ/สูตรที่คัดลอกไปเขียนใหม่ = คิดผิดตลอดไป)
   *
   *  @param {number} year ปีดิบตามที่พิมพ์บนกระดาษ (2 หรือ 4 หลัก)
   *  @param {number} shortBeFloor เส้นแบ่งปีย่อ: >= ค่านี้ = พ.ศ. ย่อ (ค่าเริ่มต้น 60)
   */
  normalizeThaiYear(year, shortBeFloor = 60) {
    const y = parseInt(year, 10);
    if (!Number.isFinite(y)) return NaN;
    if (y >= 2400) return y - 543;        // พ.ศ. 4 หลัก
    if (y >= 1900) return y;              // ค.ศ. 4 หลัก
    if (y >= 100) return y;               // 3 หลัก — ผิดปกติ ปล่อยให้ผู้เรียกตรวจเอง
    if (y >= shortBeFloor) return 2500 + y - 543;   // พ.ศ. ย่อ (69 → 2026)
    return 2000 + y;                      // ค.ศ. ย่อ (26 → 2026)
  },
