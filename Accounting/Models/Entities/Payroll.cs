...
public class SsoYearConfig : TenantEntity
{
    ...
    /// <summary>ช่วงเดือนที่แถวนี้มีผล (1–12, รวมปลายทั้งสองข้าง) — ค่าเริ่มต้น
    /// 1–12 = ทั้งปี
    ///
    /// <para>⚠️ ประกาศลดอัตราสมทบของไทยออกเป็น**ช่วงเดือน**เสมอ (เช่น 1% เดือน
    /// พ.ค.–ก.ค. 2563 · 2.5% เดือน ม.ค.–ก.พ. 2565) ไม่ใช่ทั้งปี. เดิมเก็บได้
    /// ปีละค่าเดียว ⇒ ผู้ใช้ต้องแก้แถวเดิมกลางปี ซึ่งเปลี่ยนอัตราของเดือนที่
    /// **ยื่น สปส. ไปแล้ว** ย้อนหลังไปด้วย ⇒ สร้างไฟล์ สปส.1-10 ใหม่แล้วไม่ตรง
    /// กับที่ยื่นจริง โดยไม่มีอะไรเตือน</para>
    ///
    /// <para>หนึ่งปีมีได้หลายแถว แต่ช่วงเดือนต้อง**ไม่ทับกัน**
    /// (<see cref="Helpers.SsoRateSchedule.RangesOverlap"/> เป็นตัวตรวจ)</para></summary>
    public int EffectiveFromMonth { get; set; } = 1;
    public int EffectiveToMonth { get; set; } = 12;
