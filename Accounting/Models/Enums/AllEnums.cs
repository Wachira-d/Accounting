...
/// <summary>
/// <b>วิธีบันทึก</b> เงินมัดจำ/รับล่วงหน้าฝั่งขาย — "แยก VAT เมื่อไร และลงบัญชีไหน" (รอบ 193 · คำตัดสินเจ้าของ #34
/// + คำชี้แจง "ขึ้นอยู่กับประเภทธุรกิจ · ต้องตั้งค่าได้ทั้งหมด")
///
/// ⚠️ รอบ 194: โหมด = <b>วิธีบันทึก</b> เท่านั้น · สิ่งที่กำหนดว่า "ต้องคิด VAT ไหม/เมื่อไร" คือ <b>ลักษณะเงิน</b>
/// (<see cref="DepositNature"/> บนประเภทเงินมัดจำ <c>DepositKind</c>) — มัดจำที่เป็นส่วนหนึ่งของราคา ภาษีถึงกำหนดตอนรับเงิน
/// (สินค้า §78(1)(ข) · บริการ §78/1 · ป.73/2541) ไม่ว่าจะเลือกโหมดไหน ⇒ โหมดที่เลื่อน VAT กับเงินที่เป็นราคา = ต้องมีเหตุผล +
/// คำเตือน (ไม่ปิดตัวเลือก ตามคำตัดสิน #34) · เงินประกันที่ต้องคืน = ยังไม่ใช่ tax point · นอกระบบ VAT = VAT 0 เสมอ
///
/// ลำดับชั้น (รอบ 194): ประเภทบนใบ → ประเภทของช่องทาง → ค่าเดิมของช่องทาง (<c>LodgingProperty.DepositVatTreatment</c>) →
/// ประเภทเริ่มต้นบริษัท → ค่าตั้งต้นบริษัท (<c>CompanySettings.DepositVatTreatment</c> · NULL = ตามประเภทธุรกิจ) → ประเภทธุรกิจ ·
/// ตัวตัดสินตัวเดียว = <c>Helpers/DepositPolicyResolver.ResolveKind</c> (<c>Resolve</c> เดิมคงไว้สำหรับเส้นที่ยังไม่มีประเภท) ·
/// ใบมัดจำที่ออกแล้วอ่านโหมดย้อนได้จากช่องที่ตรึงตอนสร้าง (<c>VatAmount</c> + <c>DepositOutputVatDeferred</c>) ผ่าน
/// <c>DepositPolicyResolver.OfDocument</c>
///
/// ⚠️ ค่าตัวเลขต้องคงที่ — persist ลง CompanySettings / LodgingProperties / DepositKinds
/// </summary>
...
/// <summary>
/// <b>ลักษณะเงิน</b> ของเงินมัดจำ/เงินรับล่วงหน้าฝั่งขาย — ตัวกำหนดว่าต้องคิด VAT ไหม/เมื่อไร (รอบ 194 · spec S1 ·
/// ทีมกฎหมาย L1/L2) · อยู่บนประเภทเงินมัดจำ (<c>DepositKind.Nature</c>) และตรึงลงใบตอนสร้าง (<c>Document.DepositNature</c>)
///
/// ใบเดิม (<c>Document.DepositNature</c> = NULL) = <b>ไม่ทราบ</b> — ห้าม backfill · ตัวอ่านที่ต้องใช้ลักษณะ (ริบ) ต้องให้ผู้ใช้เลือก
/// โดยค่าเริ่มต้น = ทิศปลอดภัย (คิด VAT — <c>DepositPolicyResolver.ForfeitVatDecision</c>)
///
/// ⚠️ ค่าตัวเลขต้องคงที่ — persist ลง DepositKinds / Documents · API ส่งเป็นชื่อ
/// </summary>
public enum DepositNature
{
    /// <summary>มัดจำ/เงินจอง/เงินดาวน์/ชำระล่วงหน้าที่เป็น<b>ส่วนหนึ่งของราคา</b> — สินค้า §78(1)(ข) · บริการ §78/1 · ป.73/2541
    /// ⇒ ภาษีขายถึงกำหนดตอนได้รับเงิน (ค่าแนะนำ: VatImmediate · โหมดอื่นต้องมีเหตุผล + คำเตือน)</summary>
    PartOfPrice = 1,

    /// <summary><b>เงินประกันที่ต้องคืน</b> (ความเสียหาย · กุญแจ/คีย์การ์ด · ภาชนะ · ประกันการปฏิบัติตามสัญญา) — ยังไม่ใช่ค่าตอบแทน
    /// ⇒ ไม่ใช่ tax point (ค่าแนะนำ: FullDeposit · ลงบัญชีเงินประกัน 21530/21620) · ห้ามนำไปหักเป็นฐานภาษี/ราคา
    /// (ตัดชำระหนี้ได้ด้วย ApplyDeposit เท่านั้น — <c>DEP-SEC-DEDUCT</c>)</summary>
    RefundableSecurity = 2,

    /// <summary>สิ่งที่ขาย<b>ไม่อยู่ในระบบ VAT</b> — ค่าเช่าอสังหาฯ §81(1)(ต) · ขายอสังหาฯ ที่เสียภาษีธุรกิจเฉพาะ §81(1)(น) ·
    /// สินค้า/บริการยกเว้นอื่นตาม §81 ⇒ VAT 0 เสมอไม่ว่าโหมดใด (<c>RD-81</c>)</summary>
    NonVatSupply = 3,
}

/// <summary>
/// ผู้ใช้ระบุว่า "เงินที่ริบ" เป็นอะไร — ใช้เมื่อลักษณะเงินของใบมัดจำ<b>ไม่ทราบ</b> (ใบเดิม NULL) หรือเป็นเงินประกัน
/// (รอบ 194 · spec S3) · ไม่ระบุ ⇒ <see cref="PriceOrFee"/> (ทิศปลอดภัย: จ่ายภาษีเกินมองเห็นและแก้ได้ · จ่ายขาดโดนเงินเพิ่ม 1.5%/เดือน)
///
/// ⚠️ ค่าตัวเลขต้องคงที่ · API ส่งเป็นชื่อ
/// </summary>
public enum DepositForfeitAs
{
    /// <summary>ค่าตอบแทน/ราคา/ค่าธรรมเนียมยกเลิก/ค่าของที่ใช้ไป ⇒ มี VAT (ถ้ายังไม่เคยเสีย ต้องออกใบกำกับของยอดที่ริบ)</summary>
    PriceOrFee = 1,

    /// <summary>ค่าเสียหาย/ค่าสินไหมทดแทนแท้ (ไม่ใช่ค่าตอบแทนของการขาย) ⇒ รายได้อื่นไม่มี VAT (กค 0811/5268 · ความมั่นใจกลาง-ต่ำ)</summary>
    Compensation = 2,

    /// <summary>รอบ 194 R2-2 — ใบมัดจำนี้<b>ไม่มี VAT มาแต่แรก</b> (ส่งออก 0% §80/1 · ยกเว้น §81 · ออกตอนยังไม่จด VAT) — ผู้ใช้ยืนยันได้
    /// เฉพาะใบเดิมที่ระบบแยกไม่ออกว่า "VAT 0 โดยชอบ" หรือ "มัดจำเต็มยอดที่ยังไม่เสีย VAT" (ลักษณะเงิน NULL · VAT 0 · ธงเลื่อน false) ⇒
    /// ริบแล้วไม่ออกใบกำกับ · ใบที่ธงบอกว่าเลื่อน VAT อยู่ ⇒ ค่านี้ไม่มีผล (ต้องบอก ห้าม silent no-op)</summary>
    OriginallyNoVat = 3,
}
...
/// <summary>คำยืนยันของผู้ใช้ "ตั้งใจแค่ไหน" — ตัวแยกที่กันคลังคำตอบเอียงตามนิสัยการกด
///
/// <para>ที่มา (ทีม T3 รอบ 177): ตัวปิดลูปยิงตอน<b>อนุมัติเอกสาร</b> ทุกบรรทัดที่มี
/// feedbackId โดยไม่รู้ว่าผู้ใช้มองค่านั้นหรือไม่ — <c>accepted=true</c> เมื่อค่าที่
/// ระบบเติมให้ยังไม่ถูกแตะ ⇒ ผู้ใช้ที่กด "ยอมรับและอนุมัติต่อ" เป็นนิสัยกลายเป็น
/// "ผู้ยืนยัน" วันละหลายสิบครั้ง ⇒ นักเรียนเรียนนิสัยการกด ไม่ใช่ความถูกต้อง
/// แล้วดัน Wilson ทะลุเกณฑ์ short-circuit จนไม่มีครูมาขัดอีกเลย</para>
///
/// <para>⚠️ ค่าตั้งต้นคือ <see cref="Implicit"/> โดยตั้งใจ — จุดที่<b>รู้แน่</b>ว่าผู้ใช้
/// ลงมือเลือกเองต้องประกาศเอง (เงื่อนไขที่เป็นเท็จเพราะไม่มีข้อมูล ห้ามตกเป็น "ผ่าน")</para></summary>
public enum UserChoiceSource
{
    /// <summary>ค่าที่ระบบเติมให้แล้วผู้ใช้ปล่อยผ่านตอนอนุมัติ — <b>สัญญาณอ่อน</b>
    /// (อาจไม่เคยมองด้วยซ้ำ)</summary>
    Implicit = 0,

    /// <summary>ผู้ใช้<b>ลงมือ</b>เลือก/แก้/ยืนยันค่านั้นโดยตรง (กดเลือกในดรอปดาวน์ ·
    /// แก้ช่องในฟอร์มรีวิว · ให้คะแนนคำตอบ) — สัญญาณจริง</summary>
    Explicit = 1,

    /// <summary>ยืนยันทั้งชุดในคลิกเดียว (apply-to-all · จับคู่ธนาคารทั้งก้อน) —
    /// อ่อนกว่า <see cref="Implicit"/> ในแง่ "ต่อรายการ" เพราะหนึ่งคลิกกลายเป็น
    /// คำยืนยันหลายสิบแถวพร้อมกัน</summary>
    BulkApprove = 2,
}
...
/// <summary>
/// ลักษณะของรายการเงินได้ในสลิปเงินเดือน — **ชุดที่เครื่องคำนวณภาษีต้องแยกจริง**
/// ไม่ใช่ชุดสวยตามชื่อรายการ
///
/// <para>═══ ทำไมต้องมี (DECISION_AUDIT_2026-09-18 · D6-3 · P0) ═══
/// เดิม <c>PayrollService</c> ตัดสินจาก <b>prefix ของ <c>PayrollItem.Code</c></b>
/// (<c>OT*</c> → ค่าล่วงเวลา · <c>COM</c> → คอมมิชชัน · <c>BONUS</c> → โบนัส ·
/// <b>ที่เหลือทุกตัว → เบี้ยเลี้ยงประจำ</b>) ⇒ รหัสที่ผู้ใช้ตั้งเอง (<c>BN01</c>,
/// <c>INCENTIVE</c>) ตกเป็น "ประจำ" <b>เงียบ ๆ</b> แล้วถูกฉาย × งวดที่เหลือใน
/// การประมาณการเงินได้ทั้งปี ⇒ <b>หักภาษีเกินจริงมาก</b> (โบนัส 300,000 เดือน 6
/// ทำให้หักรวม 82,221 แทน 61,925 — บั๊ก D-T3 ที่เคยแก้แล้วกลับมาทางประตูหลัง)</para>
///
/// <para><see cref="Unspecified"/> = "ยังไม่ระบุ" ต้องเป็นค่าใน enum (DOCTRINE §1 G3)
/// ไม่ใช่ null เงียบ — แถวที่เป็นค่านี้ระบบจะ<b>ตกกลับไปใช้กฎรหัสเดิม</b>
/// (<c>Helpers.PayrollIncomeNatureRules.FromLegacyCode</c>) เพื่อให้ข้อมูลเก่า
/// ที่ยังไม่ได้ migrate ได้ตัวเลขเท่าเดิมเป๊ะ</para>
/// </summary>
public enum PayrollIncomeNature
{
    /// <summary>ยังไม่ระบุ — ระบบใช้กฎรหัสเดิมแทน (คงพฤติกรรมเดิม)</summary>
    Unspecified = 0,

    /// <summary>เบี้ยเลี้ยง/ค่าตอบแทน**ประจำ** ที่คาดว่าจะได้ทุกงวด
    /// (ค่าตำแหน่ง · ค่าครองชีพ · ค่าเดินทางเหมาจ่ายรายเดือน)
    /// ⇒ <b>ถูกฉายไปงวดที่เหลือ</b> ในการประมาณการภาษีทั้งปี</summary>
    RecurringAllowance = 1,

    /// <summary>เงินได้**ครั้งคราว** ที่ไม่คาดว่าจะเกิดซ้ำทุกงวด
    /// (เบี้ยขยันเฉพาะครั้ง · เงินรางวัล · ค่าชดเชยเฉพาะกิจ)
    /// ⇒ เข้าฐานภาษีของงวดนี้ แต่ <b>ไม่ถูกฉาย</b></summary>
    OneTimeAllowance = 2,

    /// <summary>ค่าล่วงเวลา — จ่ายตามที่ทำจริง ไม่ฉาย</summary>
    Overtime = 3,

    /// <summary>คอมมิชชัน — ผันแปรตามยอดขาย ไม่ฉาย</summary>
    Commission = 4,

    /// <summary>โบนัส — ก้อนเดียว ไม่ฉาย (ม.50(1) เงินได้ครั้งคราว
    /// รวมครั้งเดียว ไม่ประมาณการซ้ำ)</summary>
    Bonus = 5,
}
