...
public interface IDocumentService
{
    ...
    /// <summary>ประวัติ revision ของใบเสนอราคา (ใหม่→เก่า) — สร้างอัตโนมัติทุก
    /// ครั้งที่แก้ใบที่อนุมัติ/ส่งแล้วผ่าน UpdateDocumentAsync.</summary>
    Task<List<DocumentRevisionListItem>> GetQuotationRevisionsAsync(Guid companyId, Guid documentId);

    /// <summary>snapshot เต็ม (JSON) ของ revision หนึ่ง — เปิดดูว่า Rev นั้นเสนอ
    /// อะไรไป (header + lines + หลักฐานการยอมรับถ้ามี). null = ไม่พบ.</summary>
    Task<string?> GetQuotationRevisionSnapshotAsync(Guid companyId, Guid documentId, int revisionNumber);
