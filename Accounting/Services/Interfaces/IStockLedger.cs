...
public sealed record StockMoveRequest(
    ...
    // true = ข้ามด่านสต็อกติดลบสำหรับ movement นี้ (Helpers/NegativeStockGuard)
    //
    // ใช้ได้เฉพาะเส้น **กลับรายการของสิ่งที่เคยบันทึกไปแล้ว** (void/ยกเลิกเอกสาร)
    // เท่านั้น: การกลับรายการไม่ใช่ "การตัดสินใจทางธุรกิจครั้งใหม่" แต่คือการลบ
    // ข้อเท็จจริงที่เคยลงไว้ — ถ้าบล็อก ผู้ใช้จะติดอยู่ในสถานะที่แก้ไม่ได้
    // (ยกเลิกใบซื้อหลังของถูกขายออกไปแล้ว = ยกเลิกไม่ได้ตลอดกาล ทั้งที่ของที่
    // ขายไปก็ไม่ได้หายไปไหน) ⇒ ด่านที่ไม่มีทางไปต่อ = ด่านที่ผิด (กฎเหล็ก #4 F2 ข้อ 8)
    //
    // ⚠️ **ห้ามใช้กับเส้นขายหรือเบิกใช้ใหม่** และห้ามกลับไปใช้ `UnitCostOverride`
    // เป็นตัวปิดด่านอีก (นั่นคือบั๊ก D5-6 ที่เพิ่งแก้: ด่านถูกข้ามโดยไม่มีใครตั้งใจ
    // เพราะช่องที่ปิดมันไม่ได้แปลว่า "ปิดด่าน")
