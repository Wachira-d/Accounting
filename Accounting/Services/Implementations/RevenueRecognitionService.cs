...
public class RevenueRecognitionService : IRevenueRecognitionService
{
    ...
    public async Task<RevenueScheduleResponse> RecognizeRevenueAsync(Guid companyId, Guid scheduleId)
    {
        ...
        // Resolve the contract's project so recognized revenue flows back
        // into project profitability — the Projects ↔ Revenue Recognition
        // link that was previously missing (Project.ActualRevenue stayed 0
        // and the revenue JE carried no ProjectId for GL-by-project).
        var projectId = await _db.RevenueContracts
            .Where(c => c.Id == schedule.RevenueContractId && c.CompanyId == companyId)
        ...
        // Roll the recognized amount into the linked project's actuals so
        // GetProfitabilityAsync and project summaries reflect real revenue
        // — mirrors how AddCostEntryAsync maintains Project.ActualCost.
        if (projectId.HasValue)
        {
            var project = await _db.Projects
                .FirstOrDefaultAsync(p => p.Id == projectId.Value && p.CompanyId == companyId);
            if (project != null)
            {
                project.ActualRevenue += schedule.Amount;
