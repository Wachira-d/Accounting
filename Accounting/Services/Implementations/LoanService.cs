...
public class LoanService : ILoanService
{
    ...
    {
        var count = await _db.Loans.CountAsync(l => l.CompanyId == companyId);
        var loanNumber = $"LOAN-{count + 1:D6}";
    ...
    public async Task<LoanResponse> UpdateAsync(Guid companyId, Guid loanId, UpdateLoanRequest request)
    {
        ...
        if (request.Notes != null) loan.Name = request.Name ?? loan.Name; // Notes field not on entity, use Name
    ...
    public async Task<LoanPaymentResponse> MakePaymentAsync(Guid companyId, Guid loanId, MakeLoanPaymentRequest request, string performedBy)
    {
        ...
        var entryCount = await _db.JournalEntries.CountAsync(j => j.CompanyId == companyId);
        var journalEntry = new JournalEntry
        {
            CompanyId = companyId,
            EntryNumber = $"LP-{entryCount + 1:D6}",
