...
public static class OcrScanComplianceEvaluator
{
    ...
    public static List<OcrScanIssueDto> Evaluate(OcrResultResponse r, string? ourTaxId)
    {
        ...
        // ── แหล่งเงินที่ระบบเดาให้เอง ─────────────────────────────────────
        //
        // ใบจ่ายเงินที่กระดาษไม่ได้บอกว่าจ่ายจากบัญชีไหน และบริษัทยังไม่ได้ตั้ง
        // บัญชีจ่ายเริ่มต้น จะได้ "บัญชีรหัสต่ำสุดที่ขึ้นต้น 111" ซึ่งมักเป็น
        // **เงินสดในมือ** ⇒ บริษัทที่จ่ายด้วยโอน/บัตรจะได้เงินสดติดลบทางบัญชี
        // และกระทบยอดธนาคารไม่เจอคู่ — เดิมค่านี้เงียบสนิท (อยู่แต่ใน trace)
        // ผลตรวจ 2026-09-06 · T1-17 (ส่วนเชิงนโยบาย "ปล่อยว่างแล้วบังคับเลือก"
        // ยังเป็นคำถามของเจ้าของโปรเจกต์ — ที่นี่แค่ทำให้ค่าที่เดาไม่เงียบ)
        if ((r.ProcessingNotes ?? "").Contains("[PAY-SOURCE-GUESS]", StringComparison.Ordinal))
            issues.Add(new("info",
                "แหล่งเงินที่แสดงเป็นค่าที่ระบบเลือกให้เอง (กระดาษไม่ได้ระบุ) — "
                + "ตรวจว่าจ่ายจากบัญชีนี้จริงก่อนอนุมัติ หรือตั้งบัญชีจ่ายเริ่มต้นในหน้าตั้งค่า"));
