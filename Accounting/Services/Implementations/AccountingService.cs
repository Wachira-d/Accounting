...
public class AccountingService : IAccountingService
{
    ...
    public async Task VoidJournalEntryAsync(Guid companyId, Guid entryId)
    {
        ...
        if (entry.Status == JournalEntryStatus.Posted)
        {
            ...
            // don't block this internal Void→Reverse cascade.
    ...
    public async Task<JournalEntryResponse> ReverseJournalEntryAsync(Guid companyId, Guid entryId, DateTime? reversalDate = null, string? description = null, bool systemTriggered = false)
    {
        ...
        try
        {
            var reversal = new JournalEntry
            {
                ...
                Status = JournalEntryStatus.Posted,
                // FIX #2: IsAutoGenerated reflects the actual trigger of THIS
                // entry — true เฉพาะตอน system cascade (VoidDocument ฯลฯ);
                // ผู้ใช้กดกลับเองในหน้าจอจะได้ false เสมอ ไม่สืบทอดจาก original.
                IsAutoGenerated = systemTriggered,
