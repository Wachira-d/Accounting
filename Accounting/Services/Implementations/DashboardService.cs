...
public class DashboardService : IDashboardService
{
    ...
    public async Task<DashboardResponse> GetDashboardAsync(Guid companyId, DashboardRequest request)
    {
        ...
        var taxSummary = await GetVatWhtSummaryAsync(companyId, fromDate, toDate);

        return new DashboardResponse(kpis, cashFlow, revenueTrends, expenseTrends,
            topCustomers, topExpenses, overdueInvoices, upcomingPayables, bankBalances, subscription, taxSummary);
    }
    ...
    private async Task<VatWhtSummary?> GetVatWhtSummaryAsync(Guid companyId, DateTime fromDate, DateTime toDate)
    {
        ...
        //   Σ(ขาย−ซื้อ) = 80,000 · Σ NetVat = 50,000 ← ต่างกัน 30,000
        // ปุ่ม preset "ไตรมาสนี้"/"ปีนี้" ใน app.html กดครั้งเดียวก็เข้าเคสนี้
        // จึงไม่ใช่ edge case
        //
        // ⚠️ **แก้บันทึกที่เคยเขียนผิด**: บรรทัดนี้เคยเขียนว่า Σ(ขาย−ซื้อ)
        // "ถูก = เงินที่จ่ายจริงรวมทั้งไตรมาส" — **จริงเฉพาะเมื่อช่วงที่เลือก
        // คลุมเดือนที่เครดิตเกิดด้วย** ซึ่งเป็นเคสเดียวที่ตัวอย่างข้างบนเลือกมา.
        // ถ้า preset ตัดเดือนนั้นออก (Q3 = ก.ค.–ก.ย. ไม่รวม มิ.ย.) เครดิตจะไหล
        // เข้ามาจากนอกช่วง ⇒ Σ(ขาย−ซื้อ) **สูงกว่า**เงินที่จ่ายจริงเท่ากับเครดิตนั้น
        // ⇒ ไม่มีสูตรไหนในสองสูตรที่ตอบ "จ่ายจริงเท่าไรในช่วงนี้" ได้ (คำตอบนั้น
        // คือ Σ max(NetVat, 0) ซึ่งเป็นคนละคำถามกับ "ยอดสุทธิของช่วง")
        // ⇒ หลายเดือนจึงติดป้าย **"สะสมในช่วง"** ไม่ใช่ "ต้องชำระ" (app.html)
        var singleMonth = fromDate.Year == toDate.Year && fromDate.Month == toDate.Month;
        decimal? netVatFromReports = null;

        // จำนวนเดือนที่ช่วงครอบ — ใช้บอกว่า "รายงานครบทุกเดือนหรือยัง"
        var monthsInRange = (toDate.Year - fromDate.Year) * 12 + (toDate.Month - fromDate.Month) + 1;
        // ⚠️ `byMonth.Count == 0` อย่างเดียวไม่พอ: ผู้ใช้สร้าง ภ.พ.30 ทีละเดือน
        // เป็นเรื่องปกติ ⇒ ไตรมาสที่มีรายงาน 1 ใน 3 เดือนจะรวมแค่เดือนเดียว
        // แล้ว **ไม่ติดป้ายประมาณการ** = ตัวเลขของเดือนเดียวใต้หัวข้อ 3 เดือน
        var isEstimate = byMonth.Count < monthsInRange;
