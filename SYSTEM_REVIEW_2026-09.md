...
| 2 | ✅ F-01 + G-01 + G-04 | XSS เชิงระบบ: `Layout.esc` ไม่หนี `"`/`'` (82 จุดใน attribute) · admin console ต่อชื่อผู้ใช้/ชื่อบริษัท/หมายเหตุสลิปดิบ 114 จุด · สมัครสาธารณะไม่ validate `FullName` · CSP `unsafe-inline` · JWT ใน localStorage ⇒ ผู้เช่า→ยึด SystemAdmin | takeover แพลตฟอร์ม | S (esc) + M (admin) |
...
1. ✅ F-02 — เก็บ URL เดิมไว้ (frontend เรียกอยู่) แล้วใส่ด่านสมาชิกที่คอนโทรลเลอร์: `DenyForeignPaymentAsync` resolve `payment.Subscription.CompanyId` → เช็ค `CompanyUsers` · `DenyForeignCompanyAsync` สำหรับ `trial/start` ที่รับ companyId ทาง body · ปฏิเสธเป็น **404 เหมือนกันหมด** กัน enumeration · ยังค้าง: R3 กลับด้าน guard (`[NonTenantScoped]` explicit) ทั้งระบบ
2. ✅ G-01 (`Layout.esc` หนี 5 อักขระ — 1 จุดปิด 82) · ✅ G-02/G-04 (`AdminLayout.esc` + ไล่ **147 จุด** ทั้ง 17 หน้าแอดมิน มากกว่า 114 ที่รายงานไว้) · ✅ G-05 (app.html 5 จุด) · ✅ G-03 (`AdminLayout.jsArg` + แก้ทั้งสองทิศ: `esc` ที่หลุดเข้า JS string 7 จุด และ `jsArg` ที่ถูกหนีซ้ำ 4 จุด) · ✅ G-06 (ตัวหนีเขียนมือ 25 ตัว → มอบต่อให้ตัวกลาง) · ✅ F-01 (`Helpers/DisplayText` ปฏิเสธอักขระเปิดแท็ก/ควบคุม + เพดานความยาว บน RegisterRequest และ Create/UpdateCompanyRequest) · checker ตัวที่ 27 `tools/escape_helper_check.py` · ยังค้าง: ถอด `unsafe-inline` ออกจาก CSP (ต้องรื้อ inline onclick ทั้งระบบก่อน — Sprint 6 G-R)
...
4. ✅ F-03 (`Helpers/UploadFileType` sniff magic bytes + นามสกุลจากผลตรวจ + ถอด SVG ทั้งชนิด + `nosniff`/`Content-Disposition: attachment` บน `/uploads/**`) · ✅ F-04 (`Helpers/OutboundUrlGuard` https-only + ปฏิเสธ IP ภายในทุกช่วง + resolve ซ้ำตอนส่งจริงกัน DNS rebinding + ปิด auto-redirect + reserved header) · ✅ F-16 · ✅ H-A6 — **Sprint 0 ปิดครบ**
...
6. ✅ C-T02/T03: `JournalEntry.IsClosingEntry` + migration backfill · `CloseFiscalPeriodAsync` ทำแค่ล็อก (ลบ `CreateClosingEntriesAsync` ทิ้ง) · กรองใบปิดออกจาก P&L/กระแสเงินสด/snapshot/CIT/ภ.ง.ด.51/ยอดที่ต้องปิดสิ้นปี · `Helpers/FiscalYear.RangeFor` ใช้ร่วม 4 ที่ + `FiscalYearRangeTests` · ยังค้าง: C-T04 `ResolveOpenFiscalPeriodAsync` (7 เส้นโพสต์เข้างวดปิดได้)
7. C-T01: predicate + Pp30Box enum (R5 ของทีม C) + แยก exempt sales/purchases
8. ✅ D-R2/D-T1..T4 — `Helpers/ThaiPitCalculator` (pure) + `ThaiPitCalculatorTests` ล็อกตัวเลขจริงทุกเคสของผลตรวจ (20,450 · 6,475 · 61,925) · หักค่าใช้จ่าย §42ทวิ ผ่าน `Section42TwiCap` ที่ไม่เคยมีใครอ่าน · ประมาณการจาก **งวดที่เหลือ** แทน `ytd×12÷เดือน` (ปิด T3+T4 ด้วยสูตรเดียว) · คืนภาษีหักเกินได้ไม่เกินที่หักไปแล้ว · 8 ฟิลด์ลดหย่อนต่อสายครบ Create/Update/Response + แท็บในฟอร์มพนักงาน
9. ✅ D-F1 (JS ส่ง ค.ศ. + ช่วงงวด · เซิร์ฟเวอร์ normalize พ.ศ. + PeriodStart/End nullable default ทั้งเดือน) · ✅ D-S1 (`IsSubjectToSocialSecurity` default = true + ช่องในฟอร์ม payroll.html + Update/Response DTO) · ✅ D-S2 (`IsActive || EndDate >= PeriodStart`) · ✅ D-S3 (`Helpers/PayrollProration` + ใช้กับทั้งเงินเดือนและฐาน ปกส.) · ยังค้าง: D-W1, D-W4, D-D1, D-X4
10. ✅ H-A1 · ✅ H-A2 (`Helpers/OutputVatRate` ตัวตัดสินอัตราตัวเดียว ใช้ทั้ง TimeBilling และ POS — ไม่ย้ายไป `CreateDocumentAsync` เพราะจะลากทั้ง service graph เข้ามาโดยไม่จำเป็นกับบั๊กนี้) · ✅ H-A3 (`ThaiDate.CalendarDateUtc` ทุกจุดที่ตัดสินวัน/เดือน: เลขบิล 3 จุด · วันที่เอกสาร · วันที่ JE · เวลาออกเลขใบกำกับอย่างย่อ) · ✅ H-A18 (throw ทั้งสองทางออกที่กลืนไว้) · ✅ H-A16 (โมดัลเลือกลูกค้า + ช่วงวัน + พาไปเปิดใบที่สร้าง)
...
12. ✅ E-OCR-01 + OCR-02 — `Helpers/ThaiVatTypeRule` เป็นกติกาตัวเดียวที่ทั้ง endpoint `/ai/vat/infer-type` (กรอกมือ) และสาย OCR ใช้ร่วมกัน · `OcrExtractedLineItem.VatRate/VatAmount` → `ExtractedItemsJson` → `OcrLineItemDto` → หน้า review · `SpreadHeaderVat` เฉลี่ยเฉพาะบรรทัดที่อัตรา > 0 (เศษลงบรรทัดที่เสียภาษีบรรทัดสุดท้าย ไม่ใช่บรรทัดท้ายสุด) · ด่าน Σ เตือนเมื่ออัตรารายบรรทัดขัดกับหัวใบ (ไม่แต่งตัวเลขให้ตรง) + `ThaiVatTypeRuleTests`
13. ✅ E-AI-06 (regex อีเมล + เลขบัญชี **ที่มีป้าย** + ชื่อฟิลด์ `bank_acct*` · **สตริงที่เป็นสมาชิกของ array ไม่เคยถูกปิดบังเลย** — ตัววนข้ามไปทั้งหมด ⇒ `top_line_items` ออกไปดิบ · `AllowTaxIdInPrompt` คงเฉพาะเลข**นิติบุคคล** (ขึ้นต้น 0) ส่วนเลขบัตรประชาชนปิดบังเสมอ · `AiPromptSanitizerTests` 16 เคส) · ✅ E-AI-07 (`pg_try_advisory_lock` — ใช้ *try* ไม่ใช่ wait เพราะการรอคือทำงานเดิมซ้ำ) · ✅ E-OCR-03 (stamp `SellerBranchCode/BuyerBranchCode = 0.30` เมื่ออ่านไม่ได้ + `fc()` เลิก fallback ไปคะแนนทั้งใบ → แสดง "—") · ✅ E-OCR-04 (`Layout.applyOcrConfidenceHints` ตัวกลาง — ใช้ทั้งฟอร์มเอกสารและ**หน้า review ที่ผู้ใช้ตัดสินใจจริง** · map ครบตาม `OcrFieldKeys`) · ✅ E-OCR-05 (VendorAddress/BuyerName/BuyerAddress/PaymentTermsDays ครบทั้ง ฟอร์ม → payload → DTO → persist) · ✅ E-AI-10 — **ตรวจแล้วเหลือ 4 ค่าไม่ใช่ 8** และ**ไม่ได้อยู่ใน `KnownTrainerFeatures`** จึงไม่เคยถูก mine (ส่วนนั้นของรายงานคลาดเคลื่อน) · ทั้ง 4 ตัวมีคู่แฝดที่ใช้งานจริง → `[Obsolete(error: true)]` ชี้ไปตัวจริง ไม่ลบเลข 13-16 ทิ้งเพื่อจองไว้

**Sprint 3 — ผู้ใช้ใหม่/nav/หน้าแรก (2–3 วัน)**
14. ✅ **เลือกทางที่สองของข้อนี้เอง: ให้ `/simple.html` โหลด `layout.js`** — ไม่ยุบสองแดชบอร์ด เพราะ B-R1 เป็น *re-design* ของหน้าแรกที่ผู้ใช้ทุกคนเห็น ส่วนที่รายงานไว้เป็น **บั๊ก** ปิดได้ครบโดยไม่ต้องรื้อ: ✅ B-A1 (หน้าแรกมี sidebar/ตัวเลือกบริษัท/ออกจากระบบ + `window.Page` ให้ `loadCompanies()` เรียกซ้ำได้) · ✅ B-A24 (`body:has(#mobileBottomNav)` ผูกช่องว่างเข้ากับการมีแถบจริง) · ✅ B-A25 · ✅ B-A19 (`Layout.toDateInput` แทน `toISOString`) · ✅ B-A15 (ข้อความชี้ FAB ที่ถอดไปแล้ว) · ✅ B-A22 (ถอด `dashboard` ออกจาก `SIMPLE_ALLOWED` — เหลือ "หน้าหลัก" อันเดียว) · ✅ B-A16 (9 จุดเดินผ่าน `dashboardUrl()`/สูตรเดียวกัน) · ✅ B-A4 (**ถอด `SIMPLE_SECTIONS` ทิ้ง** — สำเนามือชุดที่สองที่ทำให้รายการไปโผล่ใต้หมวดผิด · หมวดเป็นภาชนะ ปล่อยผ่านหมด แล้วให้ `flush()` ทิ้งหมวดว่าง ⇒ ถูกโดยโครงสร้าง) · ✅ B-A11 (3 แท็บตั้งค่าเลิกซ่อนในโหมดง่าย — วงจรปิดที่ทำให้เปิด e-Tax/ตั้งตัวย่อเลขที่ §86/4 ไม่ได้เลย) · ✅ B-A2 (CTA ใช้คำศัพท์ `?openCreate=&type=` ที่ `documents.html` อ่านจริง — ไม่สอน synonym ให้ฝั่งอ่าน) · ✅ B-A3 · ✅ B-A7 · ✅ B-A8 · ✅ B-A9 · ✅ B-A12 (`vatRegistered !== undefined` เป็นจริงเสมอเพราะ DTO เป็น `bool` ไม่ nullable) · ✅ B-A18 (นับจาก `navItems` จริง)
    · **ยังไม่ทำ**: การยุบ `/simple.html` เข้า `/app.html` (re-design) · B-A5/A6/A10/A13/A14/A17/A20/A21/A23
...
17. ✅ F-06 (`DbXmlRepository` — key ring ย้ายไป**ฐานข้อมูล** ไม่ใช่ดิสก์ต่อเครื่อง · ไม่เพิ่ม NuGet: เขียน `IXmlRepository` เองด้วย ADO.NET เพราะ DataProtection ถูกสร้างก่อน DbContext พร้อม และอ่านคีย์แบบ synchronous) · ✅ F-08 (`Helpers/SequenceNumber` + `Helpers/AssetCodeGenerator` · ยุบ 9 จุดที่ออกเลขเอง — JE 4 · Payment 2 · AssetCode 2 (สำเนาเหมือนกันคำต่อคำ) · Reservation · Booking · Order · `SequenceNumberTests` + `tools/sequence_lock_check.py` (checker ตัวที่ 28)) · ✅ F-09 (`Helpers/JobLock` — ล็อก 11 job ที่เหลือ + ยุบสำเนาใน `AiFeedbackTrainingJob` มาใช้ตัวเดียวกัน) · ✅ F-10 (ย้าย rate limiter มาหลัง `UseAuthentication()` ⇒ tier ตัดสินจากตัวตนที่ตรวจแล้ว ไม่ใช่ "มี header Authorization ไหม" · เก็บกวาดหน้าต่างที่เงียบเกิน 5 นาที) · ✅ F-11 (แคชผล BCrypt 5 นาที + เขียน `LastUsedAt` อย่างมาก 1 ครั้ง/นาที/คีย์ — สถานะเพิกถอน/หมดอายุ/IP ยังตรวจจาก DB ทุก request) · ✅ F-14 (แคชคำตอบแชทมีเพดาน 5,000 + TTL 10 นาที)
    · **ยังไม่ทำ**: F-07 (migration lock + CONCURRENTLY — ต้องรื้อลำดับ boot ทั้งก้อน ประเมินแยก) · F-12 (Puppeteer sandbox — ปิด default อยู่แล้ว) · F-13

**Sprint 5 — compliance ที่ยังไม่มี**
18. ✅ C-T04 (`RequireOpenFiscalPeriodAsync` — 9 เส้นที่โพสต์ JE ล้มดังเมื่องวดปิด แทนที่จะยัดเข้างวดที่ยื่นแบบไปแล้วเงียบ ๆ · เส้นลง JE ด้วยมือตรวจอยู่แล้วมาตั้งแต่ต้น = เดิมสองมาตรฐาน) · ✅ C-T05 (ต่อสาย `ShouldWithhold` ที่ไม่เคยมี call site — เตือนตอนอนุมัติเมื่อยอด**สะสมต่อคู่ค้าในปีภาษี**ถึง ฿1,000 แต่ใบยังไม่หัก · เตือนไม่บล็อกเพราะผู้ใช้อาจรู้ยอดสัญญาที่ระบบไม่เห็น) · ✅ C-T06 (`wht.html` สร้าง dropdown จาก `/api/reference/income-types` + **อัตราเปลี่ยนตามประเภทที่เลือก** — เดิมค้างที่ 3 เสมอ ⇒ ค่าเช่า 5% · ดอกเบี้ยบุคคล 15% · ปันผล 10% · โฆษณา 2% · ขนส่ง 1% หักผิดทุกใบ · ยุบสำเนามือทั้ง 2 ชุดในไฟล์ (dropdown + incMap)) · ✅ C-T11 (retention นับจาก**วันสิ้นรอบบัญชี** + migration `GREATEST` ไม่ย่นของใคร — เดิมสั้นไปเกือบ 12 เดือน) · ✅ C-T12 (screener §82/5 ครอบทุกทางเข้าผ่าน approval warning ไม่ใช่แค่ OCR) · ✅ C-T14 (variance คิดจากค่าที่ปัดแล้วทั้งคู่ ⇒ คอลัมน์บนจอ foot ได้) · ✅ C-T15 (ที่ดิน/งานระหว่างก่อสร้าง + วิธีคิดค่าเสื่อม → บล็อกพร้อมทางแก้ · classifier รู้อยู่แล้วแต่เส้นสร้างด้วยมือไม่เคยเรียก) · ✅ C-T16 (`AwayFromZero` — พิสูจน์ด้วยการไล่ค่า: `/2` และ `×0.15` ตกจุดกึ่งกลางจริง ส่วน `×0.20` **ไม่เคยตกเลย** ใส่ไว้เป็นการ**ป้องกัน** ไม่ใช่การแก้บั๊ก)
    · **ยังไม่ทำ**: C-T07 (ภ.พ.06 + POS) · C-T08 (ภ.ง.ด.50 export) · C-T09 (บริจาค 2%) · C-T10 (ปฏิทิน+PublicHolidays) · C-T18 (ค่าเผื่อหนี้ TFRS บทที่ 9) · C-T19 · D-S5/S6/S7 · D-U2 — ทุกตัวเป็น**ฟีเจอร์ใหม่**ไม่ใช่บั๊ก ประเมินแยกเป็นรอบของตัวเอง
...
| ✅ 34e62a3 W1 | P1 | PayrollService.cs:2972 `if (IsNullOrWhiteSpace(emp.TaxId)) continue;` · Employee.TaxId ไม่มีจุดเขียนทั้งเรพ | IssueMonthlyPnd1CertsAsync ข้ามทุกคนเสมอ — ทะเบียน cert ภ.ง.ด.1 ว่างตลอดกาล | เงื่อนไขที่เป็นจริงไม่ได้เลย | ใช้ `emp.CitizenId ?? emp.TaxId` (แบบ ExpenseClaimService:521) + เทสต์ | S |
...
| ✅ G-06 | P1 | 36 ไฟล์: audit-log.html:77 (2 ตัว) · site-settings.html:639 (ไม่หนี & + คืน '' เมื่อ 0) · pdpa.html:241 · migrate-competitor.html:62 · storefront.html:808 (5) | esc เขียนมือ 36 ชุดคุณภาพต่างกัน | resolver กลางห้ามคำนวณเอง | **ไม่ได้สร้าง `js/esc.js`** — หน้าที่โหลด `layout.js`/`admin-layout.js` อยู่แล้ว ให้ตัวหนีในหน้ามอบต่อ (`_esc(s){ return Layout.esc(s); }`) ส่วน 2 หน้าที่ไม่โหลดตัวใด (`vendor-portal.html` · `js/pay-widget.js`) หนีให้ครบ 5 ตัวในที่ของมันเอง — ได้ผลเท่ากันโดยไม่เพิ่มไฟล์ที่ทุกหน้าต้องจำว่าต้องโหลด · กันตัวถัดไปด้วย `tools/escape_helper_check.py` | M |
...
`LDG-P2-06` ✅ ค่าเช็คอินก่อนเวลา/เช็คเอาต์ช้า + **สลิปออกจาก static path สาธารณะ**
(PII — เดิมใครได้ URL ก็เปิดได้ตลอดกาล) มาอยู่หลัง endpoint ที่มีด่านสองชั้น
(`Helpers/LodgingSlipPath` + 20 เทสต์) + **LINE แจ้งเจ้าของ**เมื่อมีจอง/สลิปใหม่

**แก้บันทึกที่ผิดของตัวเอง**: รอบ 126 เขียนว่า "ไม่มีอีเมลยืนยันการจองอัตโนมัติ" —
**ผิด** · `TryNotifyAsync` ส่งอีเมลถึงแขก (created/confirmed) และเจ้าของ (created/slip)
มาตั้งแต่รอบ 124 แล้ว · ที่ขาดจริงคือ LINE เท่านั้น (ดู `LODGING_BOOKING_AUDIT.md` §6)

**พบเพิ่ม (นอกขอบเขต LDG)** — `PII-SLIP-02`: `/uploads/slips` (สลิปค่าบริการที่ผู้เช่า
โอนให้แพลตฟอร์ม) ยังอยู่ใน `publicUploadPrefixes` — คลาสเดียวกันทุกประการ ·
ตัวด่านพร้อมใช้แล้ว (`LodgingSlipPath` รับ root/prefix เป็นพารามิเตอร์)
