...
## §9 รอบ 183 — คำตัดสินของ "ทีมตัดสินใจ" 2 ชุด (main agent เปิดไฟล์ยืนยันทุกข้อ)

> โจทย์เจ้าของ 2026-09-19: **"วิเคราะห์งานคงค้าง ตั้งทีมตัดสินใจ พัฒนาให้ครบทั้งหมด"**
> ⇒ คำถามที่เคยเขียนว่า "ต้องให้เจ้าของตัดสิน" (§6.4 Q1–Q9 · §8.6 Q10–Q14) ถูกมอบให้ทีมตัดสิน
> โดยอิง `DECISION_DOCTRINE.md` §1 (G1–G7) และ `CLAUDE.md` กฎเหล็ก #2/#4 เป็นฐาน **ไม่ใช่ความชอบ**

### 9.1 กติกาที่ใช้ตัดสินทุกข้อ

ทุกคำตัดสินเลือก **"ทิศที่ความเสียหายมองเห็นและแก้ทัน"** (G5) เหนือ "ทิศที่เงียบ" เสมอ — เพราะ
ความเสียหายที่มองไม่เห็นจะถูกพบตอนสรรพากรประเมิน ซึ่งสายเกินแก้และลูกค้ารับผลเต็ม ๆ
และทุกข้อที่ "เข้มขึ้น" ต้องมี **ทางไปต่อของผู้ใช้** (F2 ข้อ 8) มิฉะนั้นถือว่ายังไม่ผ่าน

### 9.2 คำตัดสิน — ฝั่งภาษี/บัญชี

| Q | คำตัดสิน | เหตุผลชี้ขาด |
| --- | --- | --- |
| **Q1** ฐาน ปกส. | **รวมเบี้ยเลี้ยงประจำ — แต่เป็นรายรายการที่ผู้ใช้ติ๊ก** (`PayrollItem.CountsForSsoBase` เป็น `bool?` · `null` = ยังไม่ตัดสิน = **ไม่รวม + เตือน**) · งวดที่นำส่งแล้วแตะไม่ได้ | ม.5 นิยาม "ค่าจ้าง" กว้างกว่าเงินเดือนพื้นฐาน · วันนี้**นำส่งขาด = มองไม่เห็น** (สปส. ประเมินย้อนหลัง + เงินเพิ่ม 2%/เดือน ม.49) ขณะที่หักเกิน = ลูกจ้างเห็นในสลิปทันที · `null` เป็นค่าตั้งต้น ⇒ **ไม่มีลูกค้ารายไหนยอดขยับจนกว่าจะติ๊กเอง** |
| **Q2** `BuyerDeclinedTaxInvoice` | **เลิกประทับ** เหลือคำเตือน (ซึ่งมีอยู่แล้ว) — **ไม่ต้องรอ 180-A** | ✅ ยืนยันเอง: ผู้อ่านธงนี้ทั้ง 3 จุด OR กับการตรวจ "ผู้ซื้อไม่ครบ" ด้วยตัวเองอยู่แล้ว (`PdfGenerationService:1422,1615` · `TaxService:2934`) ⇒ ถอดออก **ไม่เปลี่ยนหน้าตาเอกสารแม้แต่ใบเดียว** แต่หยุดการบันทึก "เจตนาผู้ซื้อ" ที่ระบบแต่งขึ้นแล้ว clone ไปเอกสารลูกและโผล่ใน e-Tax |
| **Q3** ใบซื้อที่จะเงียบ | **ยืนยันความเงียบ + ถอดด่านเก่าในคอมมิตเดียวกัน** · **ปฏิเสธการตั้งเกณฑ์ X บาท** · แทนด้วย**หน้ารายการทบทวน "ใบที่ระบบเงียบเรื่อง WHT ประจำเดือน"** อ่านจาก audit ที่ `RecordWhtSilence` เขียนไว้แล้ว | G6: เกณฑ์ต้องมาจากการวัดช่องว่างจริง — X ที่คิดเองคือ "ค่าที่แต่งขึ้น" ซึ่ง F2 ข้อ 3 ห้ามตรง ๆ · และไม่มีมาตราใดกำหนดยอดอื่นนอกจาก 1,000 (ท.ป.4/2528 ข้อ 12) ซึ่งเป็นด่านอยู่แล้ว |
| **Q5** สถานะยื่นแบบ | **ใช้ `TaxReportStatus.Submitted` ที่มีอยู่แล้ว** (= มนุษย์ประกาศว่ายื่น · **ไม่ล็อกงวด**) · `Filed` + ล็อกงวด เฉพาะเมื่อมีเลขรับจริง | ทาง "บังคับเลขรับทันที" = ด่านที่ไม่มีทางไปต่อสำหรับคนยื่นกระดาษ · ต้นแบบที่ถูกในเรพเอง: `EtaxInvoiceService.SubmitToRevenueAsync` ไม่ประทับเมื่อไม่มี API key |
| **Q6** DTA | **ใส่โครงรอบนี้ · ไม่ใส่ตัวเลขอัตรา** — เติมแถว ม.70 (15%/10% อยู่ในตัวบท) + ตาราง `DtaTreatyRate` **ว่างเปล่า** + กติกา "ไม่มีแถว ⇒ ใช้ ม.70 เสมอ ห้ามเดาลด" + ต้องมี CoR/TH8 | หักขาดเพราะใช้อัตรา DTA ผิด = §54 ผู้จ่ายรับผิด **มองไม่เห็น** · หักเกิน = ผู้รับทวงทันที ⇒ ทิศตั้งต้นต้องเป็น ม.70 เต็ม · วันนี้อัตราที่ผู้ใช้พิมพ์ไหลลงไฟล์ ภ.ง.ด.54 **โดยไม่มีด่านเลย** |
| **Q7** ฝากขาย/POC/LCNRV/ต้นทุนแปลงสภาพ | **4 ก้อนคนละคำตอบ** (ดู 9.4 — คำถามเดิมตั้งบนสมมติฐานที่โค้ดไม่รองรับ) | — |
| **Q8** วันเริ่มค่าเสื่อม | **เพิ่ม `ReadyForUseDate` (nullable)** · สูตร = `ReadyForUseDate ?? PurchaseDate` · **ห้าม backfill** | TFRS บทที่ 10 · `null` = พฤติกรรมเดิม ⇒ ตาราง 60 งวดของสินทรัพย์เดิมไม่ขยับ (ตัวเลขที่ยื่นไปแล้วต้องไม่เปลี่ยน) · บังคับกรอกเฉพาะตอนผู้ใช้ยืนยันสินทรัพย์ที่ระบบตั้งให้ = จังหวะที่เขาเปิดฟอร์มอยู่แล้ว |
| **Q10** ทุน = 0 | **ไม่ใช่ SME** · เปลี่ยนเป็น `decimal?` · `UPDATE ... SET NULL WHERE = 0` (นิติบุคคลมีทุน 0 ไม่ได้ ⇒ 0 = ยังไม่กรอกเสมอ) + ข้อความบอกทางแก้บนรายงาน | วันนี้ "ไม่รู้" ตกเป็น SME = **เสียภาษีต่ำกว่ากฎหมาย มองไม่เห็น** · ทิศใหม่ผิด = เห็นยอดสูงผิดปกติ**ก่อน**ยื่น + มีช่องกรอกแล้ว (รอบ 182) ⇒ เงื่อนไขที่ §8.6 ตั้งไว้ว่า "รอให้มีช่องกรอกก่อน" **ครบแล้ว** |
| **Q12** รายงาน ภ.ง.ด.50 ยอดเก่า | **เตือนบนหน้าจอ + ปุ่มสร้างใหม่ · ห้าม regenerate อัตโนมัติ** · เกณฑ์ = **เทียบยอดที่คำนวณสดกับยอดที่เก็บไว้** ไม่ใช่เดาจากวันที่ · ที่ `Filed` แล้ว → "พิจารณายื่นเพิ่มเติม" ห้ามแก้ตัวเลขที่ยื่นไปแล้ว | regenerate อัตโนมัติ = ตัวเลขบนรายงานที่ยื่นไปแล้วเปลี่ยนเองเงียบ ๆ (R1 ในทรงใหม่) |
| **Q13** ยอด 11500 บวม | **ไม่ใช่ migration** — เป็น**รายงานกระทบยอด + รายการปรับปรุง (JE) ที่มนุษย์อนุมัติ** ลงวันที่ปัจจุบัน | ✅ ยืนยันเอง: `JournalEntryLine` **ไม่มี `ProductId`** ⇒ SQL ชี้ไม่ได้ว่าแถว Dr 11500 ไหนมาจากวัสดุ · ต้องไล่ `SourceDocumentId` → บรรทัดเอกสาร → `ProductType` · และงวดเก่าถูกล็อกด้วย `FilingLockedAt` |

### 9.3 คำตัดสิน — ฝั่งข้อมูล/สถาปัตยกรรม

| # | คำตัดสิน | เหตุผลชี้ขาด |
| --- | --- | --- |
| **D-1** Q14 + `ContactType.Unknown` | **ทำทั้งคู่ในคอมมิตเดียว** — เพิ่ม `Unknown = 0` + เปลี่ยน default ของ `new Contact` **แต่ไม่ backfill แถวเก่า** · 5 ใน 7 สำเนาเรียก `ContactTypeFromTaxId.Resolve` · `PayrollService` ฝัง `Individual` ตรง ๆ (พนักงาน ม.40(1) เป็นบุคคลธรรมดาเสมอ) · `DocumentService.InferContactType` ย้ายเป็น `ResolveWithBranch` (ใช้รหัสสาขาเป็นสัญญาณด้วย ซึ่ง helper ยังไม่มี) | backfill ทั้งก้อนจะ**เปลี่ยนคำตอบของ `WhtPayeeKind` fallback** ของคู่ค้าที่ไม่มีเลขภาษี ⇒ 50 ทวิ ย้ายแบบยื่นโดยไม่มีใครสั่ง · เปลี่ยนแค่ default ของแถว**ใหม่** = ไม่มีใบเก่าเปลี่ยนคำตอบแม้ใบเดียว |
| **D-2** Q11 migration | **รันแบบมีขั้นตอน** (endpoint dry-run + คนกดยืนยันต่อบริษัท + AuditLog ทุกแถว) · **และต้องทำด่านกันเกิดซ้ำก่อน**: V1 ให้ประกาศของพาร์ตเนอร์ชนะ **เว้นแต่ขัดกับทะเบียนที่ checksum ผ่าน** + ใส่ warning ให้พาร์ตเนอร์เห็น | ถ้าไม่ทำด่านก่อน การ sync รอบถัดไป**เขียนทับ migration ที่เพิ่งรัน** · ห้ามเป็น background job (ไม่มีคนรับผิดชอบ = R1 รอบใหม่) |
| **D-3** Q4 VendorIntel WHT | **ให้เติมได้ แต่แยกช่อง** — สิ่งที่ต้องตัดคือ**ลูปสอนตัวเอง** ไม่ใช่ตัวเติม · เพิ่ม `WhtEvidenceSource {None/Paper/VendorHistory/User}` · ยกเกณฑ์เป็น 0.85 + ประวัติ ≥3 ใบ · เขียน `FieldConfidence[WhtRate]` · ห้ามเรียนจากใบที่ตัวเองเติมแล้วผู้ใช้ไม่แตะ | ✅ ยืนยันเอง: ค่าที่เราเติมถูกอ่านกลับมาเป็น "กระดาษบอก" แล้วชนะทุกชั้น (ชั้น 0 ของคำตัดสินรอบ 178) · แต่ประวัติจาก**เอกสารที่อนุมัติแล้ว**คือ label ชั้นบนสุดตาม §3 — ตัดทิ้งจะเสียหลักฐานที่ดีที่สุดโดยไม่ได้แก้ลูป · ⚠️ **ห้ามลบบรรทัดที่อ่าน `scan.HasWht` ทิ้งเฉย ๆ** (กฎ H): มันรับเคสที่ engine แกะบรรทัดหักได้แต่ตัวอ่านข้อความไม่เจอ |
| **D-4** V1 Arbiter | **ต่อสาย ขั้นที่ 1** — ย้าย `DecideAll` ไปท้าย pipeline · เปลี่ยนสัญญา JSON ให้รายงาน "ค่าที่อยู่ในฟอร์มจริง + ผู้เสนอที่ตรงกัน" · ไม่มีใครตรง ⇒ `Unknown` **ห้ามเดา** · เพิ่ม `arbiterAgreed` เป็น**ตัววัด**ว่าพร้อมทำขั้นที่ 2 เมื่อไร · ถอดสำเนาตารางป้ายใน JS | ขั้นที่ 2 (ให้ arbiter ตัดสินจริง) ทำได้เมื่อ `arbiterAgreed` บนชุดกระดาษจริง = 100% เท่านั้น — ตอนนั้นการพลิกเป็น no-op ที่พิสูจน์ได้ |
| **D-5** kill-switch | **`ReorderForecast` ลบการเรียก AI ทิ้ง** เปลี่ยนเป็น narrative เชิงกำหนด (`Helpers/ReorderNarrative`) **ไม่ register student** | §2.1: ถามครูได้เมื่อครบ 4 ข้อพร้อมกัน · ตัวนี้ตกข้อ "คำตอบต้องตรวจกลับได้" (ร้อยแก้วไม่มี candidate set) และชน "ห้ามถามสิ่งที่เราเพิ่งคำนวณเอง" · register student จะได้ "ร้อยแก้วของใบเก่ามาตอบใบใหม่" ซึ่งแย่กว่าไม่ตอบ |
| **D-6** dead_helper baseline | **ซ่อม checker ก่อนตัดสินรายตัว** — แยก ORPHAN (ไม่มีใครเรียกเลย) ออกจาก INTERNAL (เรียกในไฟล์ตัวเอง = ควรเป็น private) | ✅ ยืนยันเอง: **37 จาก 65 (57%) เป็น INTERNAL** · baseline ที่ปนสองอาการ = ratchet ที่ไม่มีใครเชื่อ (F2 ข้อ 6) · **แก้แล้วในรอบนี้** (commit `9f47286`) |
| **D-7** governor | **ทำ แต่ทิศเดียว** — governor **เปิดครูเพิ่มได้ ปิดไม่ได้** + replay harness บังคับก่อน promote `Version` · ใช้เฉพาะเกณฑ์ที่มีอยู่แล้วในโค้ด ไม่ตั้งเลขใหม่ | governor ที่**ลด** sampling อัตโนมัติจะปิดช่องทางเดียวที่จะรู้ว่านักเรียนแย่ลง และผลของมัน**เงียบ** · ที่**เพิ่ม** เสียแค่ token และเห็นได้จากบิล |
| **D-8** T3-P1a | **ไม่จริง — ห้ามทำตาม** (ดู 9.4) | — |

### 9.4 ข้อที่ทีมตัดสินใจ **หักล้างเอกสารเดิม** — main agent เปิดไฟล์ยืนยันแล้วว่า**ถูกทุกข้อ**

| เอกสารเดิมเขียนว่า | ของจริง (ยืนยันด้วยการเปิดไฟล์) |
| --- | --- |
| `DOCTRINE §4.2 T3-P1a`: "~27 `AiFeatureKey` เรียก AI โดยไม่มีนักเรียน ⇒ kill-switch ไม่ผ่าน" | ทั้งระบบมีจุดเรียก `AskAsync` **24 จุด** ⇒ 27 key ที่เรียก AI เป็นไปไม่ได้เชิงเลขคณิต · ที่ไม่ผ่าน kill-switch จริง **มีตัวเดียว** (`ReorderForecast`) · ที่เหลือ (`AgingExplanation` · `ImportColumnMatch` · `ImportDataReview` · `BulkBankStatementMatch`) มี local path จริงทุกตัว · ~30 key ที่เหลือใน `KnownTrainerFeatures` **ไม่เคยเรียก AI เลย** (เป็น `Status: LocalServed, ProviderUsed: None`) ⇒ **register student ให้ทั้ง 27 ตัวคือการเพิ่มของที่ไม่มีใครเรียก** (F4 ข้อ 3) |
| `§8.6 Q11`: "ประทับผิด ⇒ ยื่น ภ.ง.ด.53 แทน ภ.ง.ด.3" | `WhtPayeeKind.Detect` เอา**เลข 13 หลักเป็นชั้น 1 เหนือ `ContactType`** ⇒ 50 ทวิ และไฟล์ยื่น**ยังแบ่งถูก** · ที่ผิดจริงคือ **e-Tax scheme ได้ `TXID` แทน `NIDN`** (`EtaxInvoiceService:1067-1076` ใช้ `ContactType` ก่อนรูปเลข) และ **ด่าน §86/4 ฟ้องผิดฝั่ง** (บล็อกผู้ซื้อบุคคลธรรมดาพร้อมข้อความ "(ผู้ซื้อนิติบุคคล)") ⇒ ยังต้องแก้ แต่ priority คนละระดับ |
| `REGRESSION_ROOT_CAUSE §ของแถม`: "`SsoRateSchedule.RangesOverlap` ไม่มีใครเรียก" | มีผู้เรียกที่ `SsoRateSchedule.cs:85` (`RangesAmbiguous`) — เป็น INTERNAL ไม่ใช่ ORPHAN · และ **57% ของ baseline เป็นอาการเดียวกัน** |
| `§3 D5`: "POC เขียน `RecognizedRevenue` ลอย ๆ ไม่มี JE" | `RevenueRecognitionService:349-384` สร้าง `JournalEntry` `Status = Posted` จริง · **ประเด็นที่ยังจริงคือ**: JE ออกจาก schedule ที่กระจาย**เส้นตรงตามเดือน** ไม่ใช่ตาม `%` ที่ผู้ใช้กรอก ⇒ ปัญหาคือ "% ไม่ขับ JE" ไม่ใช่ "ไม่มี JE" |
| `§3 D5`: "LCNRV ไม่มีโค้ด" | มี `InventoryObsolescenceAllowance` + lines + `JournalEntryId` + API + หน้าจอครบ · `Method` มี Aging/Percentage/Specific — **ขาดเฉพาะวิธี NRV** |
| `§6.4 Q7`: "ฝากขาย/POC ต่อสาย หรือ ปิดเมนู" | ทั้งคู่**ต่อสายครบแล้ว** (service + controller + หน้า + เมนู) · ปัญหาจริงคือ **ต่อแล้วให้ตัวเลขผิด**: `ConsignmentService` ตัดสต็อก**ไม่มี JE เลยทั้งไฟล์** (grep `JournalEntry` = 0) และตั้ง `VatAmount = 0m` ตายตัว · ที่ไม่มีจริงมีเพียง **ต้นทุนแปลงสภาพ** |
| คอมเมนต์ `TaxService.cs:2058-2060` | เขียนว่าเพดานค่ารับรอง = **MIN**(0.3% รายได้, 0.3% ทุน) แต่โค้ดทำ `Math.Min(10M, Math.Max(...))` ซึ่ง**ถูกตามกฎกระทรวง 143** และตรงกับ CLAUDE.md §L(4) ⇒ **คอมเมนต์ผิด โค้ดถูก** · ถ้าใครเชื่อคอมเมนต์แล้ว "แก้โค้ดให้ตรง" ลูกค้าจะบวกกลับเกินจริง |

### 9.5 บั๊กที่ทีมตัดสินใจเจอเพิ่ม (ไม่อยู่ในคำถามเดิม)

| ที่ | อาการ | สถานะ |
| --- | --- | --- |
| `contacts.html` | `String(Layout.contactTypeCode(c.contactType) \|\| 2)` ⇒ คู่ค้าที่ API ไม่ส่งชนิดมา **ฟอร์มโชว์ "นิติบุคคล"** แล้วกดบันทึกประทับค่าที่ผู้ใช้ไม่เคยเลือก · default ฝั่ง JS (2) ขัดกับฝั่งเซิร์ฟเวอร์ (1) | ✅ **แก้แล้ว** `9f47286` (เพิ่ม option "ยังไม่ระบุ" ที่มีอยู่จริง + ด่านตอนบันทึก) |
| `DocumentService` แก้ไข contact | ส่ง `taxId` มาโดยไม่ส่ง `contactType` ⇒ **ค่าที่คนตั้งไว้ถูกทับด้วยค่าที่เดา** (ละเมิด "ค่าผู้ใช้ชนะเสมอ") | 📋 รอบถัดไป (พร้อม D-1) |
| `VendorIntelligenceService` | `whtPct` คำนวณได้จาก `TotalDocuments = 1` ⇒ **ใบแรกของผู้ขังให้ `WhtConfidence = 1.0` ทันที** (ต่างจากบล็อกผังบัญชีที่มี `sizeFactor`) | 📋 รอบถัดไป (พร้อม D-3) |
| `CmsSiteService` | สำเนาที่สองของกติกาความปลอดภัย CSS ที่**หลวมกว่า**ตัวใน `Helpers/CmsSecurityHelper` (ไม่รู้จัก `@import` / `behavior:` / `-moz-binding`) | 📋 รอบถัดไป |
| `CmsSecurityHelper` 5 เมธอด | ด่าน slug/domain/HTML/JSON **มีอยู่แต่ไม่มีใครเรียก** ขณะที่ `CmsBookingService`/`CmsCommerceService`/`CmsSiteService` รับค่าดิบลง URL/โดเมน | 📋 รอบถัดไป (ORPHAN ที่มีหลักฐานว่าตั้งใจกันไว้) |
| `Pp30SalesClassifier.ShouldReport` | มีเทสต์ 6 เคสแต่ไม่มีผู้เรียก · `TaxService:473` ตัดสิน "เข้ารายงานภาษีขายไหม" ด้วย `VatAmount > 0` ⇒ **ใบส่งออก 0% (§80/1) ไม่ได้แถวรายละเอียดใน §87** ทั้งที่ยอดถูกนับเข้าช่องแล้ว | 📋 รอบถัดไป (ORPHAN ที่เป็นด่านหาย) |

### 9.6 ลำดับลงมือที่ทีมตัดสินใจวางไว้ (รอบถัดไป)

1. **D-5** (ReorderForecast) + **D-6** (checker) — ไม่แตะเงิน · ทำให้ ratchet เชื่อถือได้ก่อนงานหนัก ✅ D-6 ทำแล้ว
2. **D-3** (VendorIntel WHT) พร้อม golden ชุดกระดาษจริงก่อน/หลัง — แตะเงิน ⇒ ต้องมีตาข่ายก่อน
3. **D-1** (`ContactType.Unknown` + 7 สำเนา) — ความเสี่ยงอยู่ที่ compile เป็นหลัก (ทุก `switch` ที่ไม่มี `_ =>` จะฟ้อง CS8509) ⇒ CI คือ compiler ตัวแรก
4. **D-2 ด่านกันเกิดซ้ำ** แล้วค่อย dry-run — ด่านต้องมาก่อน migration เสมอ
5. **D-4 ขั้นที่ 1** + **D-7 replay harness** — ทั้งคู่คือ "ติดเครื่องวัด" ให้รอบถัดไปตัดสินด้วยตัวเลขแทนการเดา
6. **D-7 governor** + ORPHAN ที่เหลือ — ต้องรอ replay harness ก่อน
...
## §11 รอบ 184 — 7 ทีมผู้เชี่ยวชาญ "ถกเถียงสองฝั่งแล้วลงมือ" (main agent เปิดไฟล์ตรวจทุกข้อก่อนรวม)

> โจทย์เจ้าของ: *"ตั้งทีมที่เก่งที่สุดทุกด้านถกเถียงกัน และพัฒนาทั้งหมดเลย — ข้อ 3 คืนทั้งหมด"*
> (ข้อ 3 = "ค่าบริการควรคืนตามสัดส่วนเมื่อลูกค้าคืนอาหารทั้งจานไหม" → **คืน**)

### 11.1 คอมมิตของรอบนี้

| sha | ขอบเขต |
| --- | --- |
| `82c02a0` | POS — **คืนค่าบริการ (คำสั่งเจ้าของ)** + ด่านสิทธิ์ POS 36 จุด + checker แข็งขึ้น + เลิกกลืนความล้มเหลวซิงก์ ERP |
| `4adda50` | TEST_PLAN §0 ตามเทสต์ที่เพิ่ง add (CI ฟ้องถูก) |
| `6e95175` | ภาษี + เงินเดือน |
| `b0ca78c` | ธนาคาร / เงินสดย่อย / เช็ค |
| `723f4ea` | OCR / AI |
| `cee9b73` | ข้อมูล / ทางเข้าภายนอก (V1 · นำเข้า-ส่งออก) |
| `555cf16` | บรรทัดเอกสารห้ามติดลบ + ปิด P0 ออเดอร์หน้าร้านที่ไม่เคยเข้าบัญชี |
| `453f1e9` `9167816` | ซ่อม build ที่ CI จับได้ 2 รอบ → **CI เขียว** |

### 11.2 **9 ข้อที่ต้องให้เจ้าของตัดสิน — ห้ามเดาแทน**

| # | เรื่อง | ทางเลือกและผลต่าง |
| --- | --- | --- |
| 1 | **เงินสดย่อยไม่มีใบเสร็จ** — ทีมเลือก "บันทึกได้ + ติดธง `IsNonDeductible`/`RD-65TER-9`" แต่ `CLAUDE.md` §L(9) เขียนว่า **hard block** | เงินออกลิ้นชักไปแล้วจริง ถ้าห้ามบันทึก ยอดในระบบจะไม่ตรงกับเงินในลิ้นชัก **ถาวรและมองไม่เห็น** · ธงไหลเข้า worksheet บวกกลับ ภ.ง.ด.50 เอง ⇒ ความเสียหายถูก**นับ** ไม่ใช่ถูก**ซ่อน** · **ถ้าต้องการ block จริง สลับได้ที่จุดเดียว** |
| 2 | **คืนทิปหรือไม่** (วันนี้ไม่คืน) | ทิปเป็นหนี้สิน 21814 ที่ถือแทนพนักงาน และมักจ่ายออกผ่าน `TipPayoutService` แล้ว ⇒ คืนแล้วบัญชีติดลบ = บริษัทออกเงินแทนพนักงานเงียบ ๆ · เปิดได้ แต่ต้องมี `RefundTipAmount` + ขา JE `Dr 21814 / Cr เงินสด` + ด่านว่าทิปของกะนั้นยังไม่ถูกจ่ายออก |
| 3 | **ค่าจัดส่ง CMS + บรรทัดปัดเศษ POS ยัง `VatRate = 0`** ⇒ ไหลเข้า ภ.พ.30 **ช่อง 7 (ยอดส่งออก §80/1)** ซึ่งผิดความหมาย | แก้ได้ แต่**กระทบออเดอร์/บิลที่ไม่มีส่วนลดด้วย** = ผิดข้อบังคับ "เอกสารปกติห้ามขยับ" ที่ใช้คุมรอบนี้ ⇒ ต้องเป็นรอบของตัวเองพร้อมชุดกระดาษจริงก่อน/หลัง |
| 4 | **`SyncOrderToErpAsync` ใช้ `DocumentDate = UtcNow`** ไม่ใช่วันตามปฏิทินไทย ⇒ tax point §78 ผิดงวดเมื่อจ่ายช่วง 00:00–07:00 ICT | ต้องตัดสินพร้อมนโยบาย backfill ออเดอร์ที่จ่ายแล้วแต่ค้างไม่มีเอกสาร · กติกาที่เสนอ: งวดยังเปิด ⇒ sync อัตโนมัติด้วยวันที่จ่ายจริง (ICT) · งวดปิด/ยื่นแล้ว ⇒ **ห้าม** sync เอง ให้ผู้ใช้เลือกยื่นเพิ่มเติม ภ.พ.30 หรือลงงวดปัจจุบันพร้อม `LateReason` |
| 5 | **`ImportDocumentAsync` ไม่เดิน `CreateDocumentAsync`** | ถ้าบังคับให้เดินด่านเต็ม ข้อมูล migration ของเก่าอาจตกด่าน §86/4 **ทั้งก้อน** — ยอมให้เดินด่านเต็มไหม หรือทำโหมด "นำเข้าของเก่า" ที่บันทึกเหตุผลไว้ |
| 6 | **PDPA** — DSR endpoint ครบทั้ง 4 ตัวแต่ `pdpa.html` **ไม่เคยเรียกสักตัว** ⇒ สิทธิ์ ม.30/31/33 ใช้จริงไม่ได้ · `PiiAccessLog` มีผู้เรียกจริง 2 จุด | ต่อสาย UI + ลง log ให้ครบเป็นงานรอบของตัวเอง (control เชิง compliance ที่ไม่มีเทสต์ = ไม่มี control ⇒ ทีม**ตั้งใจไม่ใส่ครึ่งใบ**) |
| 7 | **endpoint ที่เขียนข้อมูลแต่ยังไม่มีด่านสิทธิ์ 56 จุด** — CmsCommerce 19 · Integration 24 · CmsBooking 9 · Mobile 4 | **บางตัวเป็น endpoint สาธารณะของหน้าร้าน** ใส่ด่านมั่ว = ร้านพัง ⇒ ต้องไล่ทีละตัวว่า "ลูกค้าปลายทาง" หรือ "พนักงาน" |
| 8 | **ย้าย POS เข้าเส้นเอกสารกลาง** (แผน 4 เฟส) | เฟส 1 ต้องตัดสินเรื่อง**โควตา**ก่อน: ร้านอาหาร 300 บิล/วัน = 9,000 เอกสาร/เดือน เทียบกับแพ็กทดลองที่ตั้งไว้ 20 |
| 9 | **§76 เพดานหักค่าจ้าง 1/5 + `EmployeeConsentAt`** | สเปกเขียนครบแล้ว ยังไม่ลงมือ — ต้องตัดสินว่าบังคับย้อนหลังกับรายการหักที่ตั้งไว้แล้วหรือไม่ |

### 11.3 ข้อจำกัดที่ต้องรู้เมื่ออ่านผลรอบนี้

- **เครื่องพัฒนาไม่มี .NET SDK** ⇒ CI บน `claude/**` คือคอมไพเลอร์ตัวแรก · รอบนี้ CI จับ build error **2 รอบ**
  (`using` ใต้ namespace · `User.CompanyId` ที่ไม่มีจริง) ก่อนจะเขียวที่ `9167816`
- **`dotnet test` ยังไม่เคยรันบน branch นี้** — job `test` รันเฉพาะ PR เข้า main / push main / `workflow_dispatch`
  และ `workflow_dispatch` ตอบ **403** ให้ token ของ agent ⇒ เทสต์ที่เพิ่มรอบ 183–184 (**44 ไฟล์ใหม่**)
  **คอมไพล์ผ่านแล้ว แต่ยังไม่เคยถูกรัน** · ทางได้ผลจริง: เจ้าของเปิด PR เข้า main หรือรัน `dotnet test` ในเครื่องตัวเอง
