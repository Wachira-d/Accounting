...
| **เครดิต NextAcc ท้ายเอกสาร (เฉพาะบัญชีแพ็กเกจฟรี)** | ข้อความจาง ๆ 2 บรรทัดมุมขวาล่างทุกหน้า: "จัดทำด้วย **NextAcc** · ระบบบัญชีออนไลน์ / เริ่มใช้ฟรีที่ www.nextacc.net" (7–7.5pt สีเทา ไม่แย่งสายตาจากเนื้อหา). **เกณฑ์ตัดสิน `PdfGenerationService.IsFreeTierAsync` = "ไม่ได้จ่ายเงิน"** (ไม่ใช่แค่ "ทดลองใช้") — เข้าเงื่อนไขเมื่อข้อใดข้อหนึ่งจริง: (1) แพ็กเกจ = `FreeTrial` · (2) **`PlanTemplate.IsPermanentFree`** — ตัว**แพ็กเกจ**ถูกตั้งเป็นฟรีตลอดชีพ (ความจริงระดับแพ็กเกจที่ admin ตั้งเอง) · (3) ธง `IsPermanentFree` บน**แถว subscription** **+ `PricePerCycle = 0`** (ดีลฟรีเฉพาะราย). บริษัทใต้ License ดูที่ชั้น License: `PlanTemplate.Plan` / `PlanTemplate.IsPermanentFree` / ราคาที่ตกลง (`MonthlyPrice+AnnualPrice = 0`). **ทำไมข้อ 3 ต้องมีเงื่อนไขราคาประกบ**: ธงบนแถว subscription เชื่อลำพังไม่ได้ — ข้อมูลจริงมีแถวแพ็กเกจเสียเงินที่ธงนี้ค้างอยู่ (บั๊ก sync ที่แก้ไปรอบ 10 แต่แถวเก่ายังค้าง) ถ้าเชื่อธงเดี่ยว ๆ ลูกค้า Enterprise ที่เพิ่งจ่ายเงินจะมีโฆษณาโผล่บนใบกำกับของเขา; แถวที่ค้างเหล่านั้นมี `PricePerCycle > 0` เสมอ (ตั้งตอนอนุมัติการชำระ) ⇒ เงื่อนไขราคาคัดออกพอดี ขณะที่ดีลฟรีจริงราคาเป็น 0 อยู่แล้ว. อ่านข้อมูลไม่ได้/ไม่มี subscription/หา License ไม่เจอ → **ไม่พิมพ์** (พลาดฝั่ง "ไม่โฆษณา" ปลอดภัยกว่าพลาดฝั่ง "โฆษณาใส่คนจ่ายเงิน"). **ต่อสายครบทุกทางออก** — คิดค่าครั้งเดียวใน `GenerateDocumentPdfAsync` แล้วส่งเข้า: e-Tax PDF/A-3 · Chromium HTML→PDF · QuestPDF · เส้นสำรอง HTML→QuestPDF · print preview (`GenerateDocumentHtmlAsync`) · พรีวิวเทมเพลตทั้ง 3 ทาง — ถ้าลืมเส้นใดเส้นหนึ่ง เอกสารจะมี/ไม่มีเครดิตไม่เหมือนกันแล้วแต่ว่าออกทางไหน. **ไม่แปะบนแบบฟอร์มราชการ** (50 ทวิ / ภ.พ.30 ฯลฯ) — เป็นแบบของกรมสรรพากร ห้ามเติมข้อความของเราเอง |
...
_Last verified against codebase: 2026-08-14 (รอบ 18: เครดิต NextAcc ท้ายเอกสารเฉพาะ_
_บัญชีที่ไม่ได้จ่ายเงิน — ทดลองใช้ + ฟรีตลอดชีพ (ดูที่ PlanTemplate) + ดีลฟรีราคา 0;_
_ธง IsPermanentFree บนแถว subscription เชื่อได้ต่อเมื่อ PricePerCycle=0 เพราะมีแถว_
_แพ็กเกจเสียเงินที่ธงค้าง · ต่อสายครบทุก renderer · ไม่แปะแบบฟอร์มราชการ)_
