...
- **โซนผู้ขาย/ผู้ซื้อ · วันที่ · สาขา · เล่มที่ (รอบ 190 — เพิ่มขั้น ไม่รื้อ · ตัวตัดสินทุกตัวเป็น pure helper มีเทสต์ข้อความกระดาษจริง)**:
  ...
  - **ผู้ติดต่อตามสาขา** (รอบ 197 · คำตัดสินเจ้าของ "คนละสาขา คนละที่อยู่ = คนละผู้ติดต่อ") — `Helpers/OcrVendorBranchContact.Decide`
    ตัดสินด้วย **`ContactTaxBranchKey.PickBranch` ตัวเดียวกับทุกทางเข้า** (§6.2i): สาขาตรง ⇒ แถวนั้น · แถวไม่เคยระบุสาขา ≡ สนญ. (อ้างได้เฉพาะใบ สนญ.) ·
    **สาขาบนกระดาษมีหลักฐาน (คะแนน ≥ 0.85 · `IsReliableBranch`) แต่ไม่ตรงแถวใด ⇒ `NewBranchRow`: สร้างผู้ติดต่อแถวใหม่ของสาขานั้น**
    (`NewVendorBranchContactAsync` — ชื่อนิติบุคคลจากทะเบียน → แถวแม่แบบ สนญ. (ตัดป้ายสาขา) → กระดาษ · สาขา/ชื่อสาขาจากทะเบียน VAT · ที่อยู่ของสาขาผ่าน
    `OcrIssuerBranch.ContactAddress` · `CreatedBy = OCR-BranchAutoCreate`) และ**ห้ามถอยไปจับด้วยชื่อ/AI** (จะได้แถว สนญ. คืน) · หลักฐานอ่อน (ขัดกับประโยค
    บนกระดาษ 0.50) ⇒ `OtherBranchRow` ผูกแถวเดิมแต่ไม่เติมข้ามสาขา · อ่านสาขาไม่ได้ ⇒ กติกาเดิม (สนญ. ก่อน) ไม่สร้างแถว · เดิม (รอบ 190) ไม่มีแถวสาขาตรง ⇒
    ผูก สนญ. เงียบ ๆ (ใบ Makro สาขาชลบุรี 00005 ผูกผู้ติดต่อ 00000) · **เส้นสร้างเอกสาร** (`CreateDocumentFromScanCoreAsync` ฝั่งผู้ขาย) ตัดสินซ้ำด้วยตัวเดียวกัน
  ...
  - **ชื่อผู้ขายเป็นโลโก้** (รอบ 197 · ใบ Makro "ma ro") — (ก) เลขผู้เสียภาษีที่พิมพ์แบ่งกลุ่มแบบอื่น ("0 10 7 567 00041 4") ที่มีป้ายกำกับ ⇒
    ...
    `Helpers/OcrVendorLegalName.FindAboveTaxId` ใช้บรรทัดชื่อนิติบุคคลที่พิมพ์เหนือเลข (≤ 6 บรรทัด · ไม่อยู่บล็อกผู้ซื้อ · ไม่ใช่ชื่อเรา · 0.85) **ก่อน** sync
    `ExtractedVendorName`/ตัวเรียนรู้ Azure/ทะเบียน (ค) ทะเบียนไม่ยืนยัน ⇒ `PickKnownLegalName` ชื่อที่มีรูปนิติบุคคลของเลขเดียวกัน (คลัง UserCorrection →
    ผู้ติดต่อ สนญ. ก่อน → คลังอื่น · 0.70 ไฮไลต์)
  ...
  - **ส่วนลดท้ายบิล / VAT ผสม (ทีม M)** — ตัวอ่านส่วนลดตัวเดียว `Helpers/OcrBillDiscount` (เดิมหยิบ "ส่วนลด" ตัวแรกของหน้า ⇒ ได้
    ...
    **ลำดับตั้งอัตรา VAT รายบรรทัด** (`OcrService.BuildScanLinesAsync` — ตัวสร้างเดียวของสร้าง/line-preview/ดึงรายการซ้ำ · เติมเฉพาะ
    บรรทัดที่ยังว่าง): (1) ค่าจาก engine/ผู้ใช้แก้ในหน้ารีวิว → (2) สัญลักษณ์บนกระดาษ (V/N · ตารางสรุป VAT) ที่ยอดพิสูจน์ได้
    (`Helpers/OcrLineVatMarks`) → (3) **ตัวเลขหัวใบพิสูจน์ทั้งใบ** (`Helpers/OcrLineVatPlanner.PlanWholeInvoice` · รอบ 195 — VAT หัวใบ =
    7% ของฐานหลังลด (หรือ 7/107 ของยอดรวมเมื่อราคารวม VAT) ±ครึ่งสตางค์/บรรทัด · Σ บรรทัดหลังกระจายส่วนลด = ฐาน · ยอดยกเว้นบนกระดาษ
    0/ไม่มี (`PaperExemptAmount` = แถว "ยกเว้น/ไม่ต้องเสียภาษี" ∪ กลุ่มไม่ใช่ 7% ในตารางรหัส ภ.พ.) · ชั้นบนตั้งอัตราไม่ใช่ 7% ไว้ = ไม่ตัดสิน ·
    **VAT หัวใบต้องพิมพ์บนกระดาษในฐานะ VAT** — `Helpers/OcrHeaderVatEvidence.Classify` = `Labelled` (แถวป้าย VAT · ตารางกลุ่ม · ป้ายแล้วตัวเลข
    บรรทัดถัดไป `OcrPaperAmounts.IsVatLabelled` · e-Tax XML) — VAT ที่ระบบแยก 7/107 เองผ่านเลขคณิตทุกครั้งโดยการสร้าง จึงไม่ใช่หลักฐาน (รอบ 195 ฝ่ายค้าน C1))
    ...
    **ไม่บล็อก ไม่เปลี่ยนค่า** — รอเจ้าของตัดสินว่าจะบังคับไหม) · อยู่ชุดเดียวกับ `[Σ-GAP]` (`IsGapWarning`): เว็บ/มือถือต้องกดรับทราบ · workflow/ลายเซ็นส่งผ่านเองไม่ได้ ·
    ...
    พร้อมตัวเลขก่อนกดสร้าง (เดิมป้ายเขียว "ผลรวมตรง" โดยไม่ดูผลตรวจ) · **ตอนอนุมัติ** (`CollectApprovalWarningsAsync` →
    `OcrApprovalGapWarning.Build`): ท่อน "ตอนนี้รายการรวม…" อยู่ข้อแรกข้อเดียว · ถ้า `OcrLineVatPlanner.RateAdvice` พิสูจน์ได้ว่าบรรทัด
    "ตอนนี้" ไม่เป็น 7% ทั้งที่ VAT กระดาษ = 7% ของฐานทั้งใบ ⇒ ข้อยอดรวม/VAT/อัตรา รวมเป็นข้อเดียว "ตั้งอัตรา VAT บรรทัดเป็น 7% (VAT หัวใบ X =
    7% × ฐาน)" (ยังต้องกดรับทราบ · ข้ออื่นแยกเหมือนเดิม) · คำแนะนำนี้ไม่ออกเมื่อ VAT หัวใบไม่ได้พิมพ์ในฐานะ VAT · ข้อ**ยอดรวม**รวมเข้าได้เฉพาะเมื่อ
    ตั้ง 7% แล้วยอดรวมตรงกระดาษ ±0.05 (`SameRootTolerance`) ไม่งั้นแยกไว้พร้อม "ยังต่างกระดาษ X" (ค่าธรรมเนียม/ค่าส่งที่อ่านไม่ได้)
...
- **Behavior พิเศษ**:
  ...
  - **ผู้ซื้อไม่ประสงค์รับใบกำกับภาษี (ขายปลีก)** — 2 ทางเข้า:
    ...
    **หลังฝ่ายค้านรอบ 194 (C1/C4/P6 · `review194-regsec.md`)**: ช่องมัดจำที่เป็นราคา (มัดจำค่าห้อง `priceChannel: true` · ชำระล่วงหน้า CMS) และ
    **ชั้น ④ ประเภทเริ่มต้นบริษัทเสมอ** ข้ามประเภทที่ลักษณะไม่ผ่าน `DepositPolicyResolver.AcceptableAsPriceDeposit` (ราคา/นอกระบบ VAT) ⇒ ตกชั้นถัดไป +
    คำเตือน `DEP-KIND-NATURE` (ไม่ข้ามเงียบ) · ประเภทที่ผู้ใช้เลือกบนใบเอง (ชั้น ①) ไม่ถูกกรอง · `DepositKindService.SetDefaultAsync` ปฏิเสธเงินประกัน
    (`DepositKindCatalog.DefaultKindProblem` · หน้าเว็บซ่อนปุ่มด้วย `CanBeDefault`) · `UpdateAsync` เปลี่ยนลักษณะไม่ได้เมื่อมีใบอ้างแล้ว / ผูกกับที่พักแล้วจะขัดช่อง /
    เป็นค่าเริ่มต้นแล้วจะเป็นเงินประกัน (`DepositKindCatalog.NatureChangeProblem` — ทางไปต่อ: สร้างประเภทใหม่) · migration `DepositKindSecurityDefaultFixSql`
    ถอดธงเริ่มต้นของประเภทเงินประกันที่ตั้งไว้ก่อนมีด่าน แล้วคืนให้ ADVANCE (seed) เฉพาะบริษัทที่ถูกถอดและไม่มีค่าเริ่มต้นอื่น (ไม่แตะ "Documents") ·
    เหตุผลนโยบาย (`DepositPolicyNote`) = **บันทึกภายในของใบ + คำเตือนตอนอนุมัติ ไม่พิมพ์บนเอกสาร** (renderer ทั้งสองไม่อ่าน — ข้อความหน้าตั้งค่า/`KindProblem`
    แก้ให้ตรง) · schema รอบ 194 (ตาราง `DepositKinds` + index + ADD COLUMN บน Documents/ที่พัก) อยู่ใน `GetAlterStatements` (เส้นที่ log) และซ้ำในชุดหลังก่อน seed
    (`DepositKindSchemaStatements`/`DepositKindSeedStatements`) · ชุดหลัง (`ApplyFullTextSearchIndexes`) log ความล้มเหลวที่ไม่ใช่ "มีอยู่แล้ว" แทน `catch {}`
...
- **Booking flow** (`CmsBookingService.SyncBookingToErpAsync`):
  - Map `BookingType` → DocumentType:
    ...
    - อนุมัติอัตโนมัติ (ทุกชนิดที่ไม่ใช่ Lead) ล้ม ⇒ **ถอยของที่ขั้นอนุมัติทิ้งไว้ใน context** (`TrackedChangeRevert` · รอบ 194 R3 P-1 — เดิม catch แล้ว
      `SaveChanges` ต่อ ⇒ ใบ Approved ไม่มี JE) · ใบคงเป็นร่าง + หมายเหตุ ⚠️ บนการจอง ("เปิดเอกสารแล้วกดอนุมัติ") + log
    ...
    - **ประเภทเงินมัดจำ (รอบ 194 ทีม C · spec S4)**: ส่ง `DepositKindId:` = ประเภทเริ่มต้นบริษัท **เฉพาะเมื่อบริษัทตั้งค่ามัดจำเองแล้ว**
      ...
      ได้ใบเดิมทุกตัวอักษร (VAT ทันที §78/1) · ตั้งแล้ว ⇒ DocumentService จัดรูปใบผ่าน `DepositDocumentShaping` ตัวเดียว · ตัดสินที่
      `DepositKindCatalog.PrePaymentKindId` ตัวเดียว — ค่าเริ่มต้นที่เป็นเงินประกันไม่ถูกส่ง (ฝ่ายค้าน C1: ใบจองลักษณะเงินประกัน ⇒ ใช้บริการแล้ว `SecurityNotRevenue` ไม่รับรู้รายได้)
...
- **รอบ 194 ทีม B — เส้นเอกสารต่อสายประเภทเงินมัดจำแล้ว** (spec `erp-review/2026-09-25/spec-194.md` S2–S4 · กติกาเส้นเอกสาร
  ...
  - **รอบ 194 ทีม M (หลังฝ่ายค้านเงิน/ภาษี `erp-review/2026-09-25/review194-money.md` M1–M6 + PLAUSIBLE · regsec C2/P1 ·
    เทสต์ `DepositForfeitRound194MTests` สองครึ่ง · ล็อกจุดเรียก บล็อก "รอบ 194 ทีม M")** — แทนที่พฤติกรรมในย่อหน้าก่อนหน้าตามนี้:
    - **"ริบ" = `RealizeDepositRequest.ForfeitAs` ระบุ (M5)** — `ForfeitVatDecision` ถูกเรียกเฉพาะตอนริบ · ไม่ระบุ = **รับรู้ตามปกติ** (ส่งมอบ/ให้บริการแล้ว):
      บัญชีที่ผู้เรียกระบุ → 41000 → 42000 (บัญชีริบของประเภท**ไม่ทับ**) · คำอธิบาย JE "รับรู้รายได้จากมัดจำ" · มัดจำเต็มยอด (VAT 0 + deferred) ที่เป็นราคา/ไม่ทราบลักษณะ
      ของบริษัทที่จด VAT ⇒ ปฏิเสธ `DEPOSIT-PLAIN-REALIZE` พร้อมทางไปต่อ (ออกใบกำกับแล้ว "หักมัดจำ" · หรือเลือก "ริบมัดจำ") — ตัวตัดสิน
      `DepositPolicyResolver.PlainRealizeProblem` ตัวเดียว (หน้าเว็บแสดง `DepositSummary.PlainRealizeBlockedReason` ก่อนกด) · หน้าต่างรับรู้ (`deposits.html`/
      `deposit-center.html`) มีตัวเลือก "ส่งมอบ/ให้บริการแล้ว" (ค่าเริ่มต้น · ไม่ส่ง `forfeitAs`) กับ "ริบมัดจำ" (ส่งค่าที่เลือก ไม่มีตัวเลือก = `PriceOrFee`)
    - **บัญชีรายได้** `DepositPolicyResolver.RevenueAccountPlan` ตัวเดียว: ค่าผู้ใช้ชนะเสมอ · ริบนอกระบบ VAT = ที่ระบุ → `ForfeitAccountCode` → 41000 ·
      **ริบเป็นค่าเสียหาย = ที่ระบุ → `ForfeitAccountCode` → 43080 (รายได้ค่าปรับ/ค่าเสียหายที่ได้รับ) → 43070 → ล้มดัง `DEPOSIT-FORFEIT-ACCOUNT`** (ห้ามตกไป
      รายได้ขาย 41000/ค่าห้อง — ไม่มีใน ภ.พ.30 · P-c) · คำอธิบาย JE ตาม action จริง ("ริบมัดจำเป็นรายได้ (ไม่มี VAT)" เฉพาะค่าเสียหาย/นอกระบบ VAT)
    - **VAT 0 โดยชอบ (M2)** — `ForfeitVatDecision(..., depositOutputVatDeferred:)`: VAT 0 + ไม่ deferred (ยกเว้น §81 · 0% §80/1 · ออกตอนยังไม่จด · ช่องทางไม่คิด VAT)
      ⇒ `ZeroVatAtIssue` (ไม่มี VAT · ไม่ออกใบกำกับ · ไม่ติดธง · ไม่ถามตัวเลือก) · VAT 0 + deferred = มัดจำเต็มยอด ⇒ ออกใบกำกับตามเดิม · ผู้เรียกที่ไม่ส่งธง (null) = ถือว่าเลื่อน
    - **P1 (regsec) อัตราของช่องทาง** — `CreateDocumentAsync(..., depositChannelVatRate:)` (พารามิเตอร์เมธอด ไม่ใช่ช่องใน request · ที่พักส่งอัตราของที่พัก)
      ⇒ ตัวจัดรูปใช้ `ShapingVatRate(บริษัท, ช่องทาง)` — ที่พัก `ChargeVat=false` คงรูป VAT 0 ไม่เลื่อน (เดิมถูกจัดซ้ำเป็นเต็มยอดด้วยอัตราบริษัท ⇒ ริบได้ใบกำกับ 7%) ·
      `RealizeDepositAsync(..., channelVatRate:)` (ที่พักตอนยกเลิก) ⇒ `ForfeitZeroVatDeferred` ถือใบ VAT 0 ของช่องทางที่ไม่คิด VAT เป็น "VAT 0 โดยชอบ" (ครอบใบเก่าด้วย)
    - **VAT พักที่ย้ายเข้า 21911 = ยอดที่ยังพักจริงใน GL (M3)** — `PendingUndueVatInGlAsync` + `UndueVatToRecognize` ทั้งเส้นรับรู้และปุ่ม "🧾 VAT"
      (`RecognizeDepositOutputVatAsync`) · ไม่เหลือ = ไม่ย้าย/ไม่ประทับ · ภ.พ.30 แถวมัดจำ VAT พักรายงานเท่าที่ Cr 21911 จริง (`ReportedRecognizedDepositVat`/`…Base`
      ใน `TaxService.GenerateVatReport`) · ตัวอย่าง 1,070/70: ค่าเสียหายฐาน 500 (Dr 21913 35) แล้วรับรู้ส่วนที่เหลือ = ย้าย 35 · รายงาน 500/35 (เดิม 70 ⇒ 21913 ติดลบ 35)
    - **tax point ของ VAT ที่เกิดจากการริบ (M4)** — `ForfeitTaxPointDecision(lateVat, วันรับเงิน, วันที่ริบ, งวดเดือนรับเงินยื่น/ล็อกแล้ว)` (`VatPeriodDeclaredOrFiledAsync` =
      `DeclaredOrFiledStatuses` หรือ `FilingLockedAt`): เดือนเดียวกัน / งวดยังไม่ยื่น ⇒ tax point = วันรับเงิน (ใบกำกับส่ง `PaymentDate` ⇒ `TaxPointResolver` ได้ MIN ·
      VAT พักประทับ `DepositOutputVatRecognizedAt` = วันรับเงิน) **ไม่มีธง** · งวดยื่นแล้ว ⇒ งวดปัจจุบัน + ธง `[DEPOSIT-LATE-VAT]` บอกว่า "นำส่งในงวด {เดือนนี้} แล้ว
    ...
    - **ริบบางส่วนหลายครั้ง (C2 regsec)** — `ApplyDepositToInvoiceCoreAsync` ผ่อน one-shot เฉพาะเมื่อใบเดิมหรือใบปลายทางเป็นใบกำกับของการริบของมัดจำนี้
      (`ApplyToAnotherTargetAllowed`) · ยกเลิก/ลบใบปลายทางหามัดจำจาก JE ตัดชำระด้วย (`DepositsAppliedToAsync`) และปลดตัวชี้เฉพาะเมื่อชี้ใบนั้น
    - **คำเตือน 21913 ค้างเกิน 90 วัน (M6)** กรอง `DepositPolicyResolver.UndueVatStillOpen` (ไม่ฟ้องใบที่รับรู้/ริบ/คืนครบ)
    - **P-e** ด่านเงินประกัน (`GuardSecurityDepositDeductionAsync` · `LoadTaxedDepositsByRefAsync` · integration `DepositKindPayloadRejectionAsync`) ตัดสินจาก
  ...
  - **รอบ 194 ทีม M3 (หลังฝ่ายค้านรอบสาม `erp-review/2026-09-25/review194-r3.md` R3-1/R3-2 + P-1/P-2/P-4 · เทสต์ `DepositRound194R3Tests` สองครึ่ง ·
    ล็อกจุดเรียกบล็อก "รอบ 194 R3-2" ใน `tools/required_call_site_check.py`)** — แทนที่ข้อ R2-1/P-a ข้างบนตามนี้:
    - **R3-1 tax point สองเส้นแยกกัน** — `ForfeitTaxPointDecision(DepositForfeitVatRoute route, …)` (ผู้เรียกต้องระบุเส้นเสมอ):
      `ForfeitTaxInvoice` (ออกใบกำกับของยอดที่ริบ) ⇒ tax point = **วันที่ของใบ (วันริบ) เสมอ** · `IssueForfeitTaxInvoiceAsync` ส่ง `PaymentDate: null`
      (เดิมส่งวันรับเงิน ⇒ `TaxPointResolver` ได้เดือนรับเงิน แต่ JE ของใบ `EntryDate = DocumentDate` ลงเดือนริบ — มัดจำ 10,700 รับ 20/01 ริบ 05/02:
      Cr 21911 700 ลง ก.พ. · ภ.พ.30 ม.ค. +700 · ใบลงวันที่ ก.พ. อยู่ในรายงานภาษีขาย ม.ค. ผิดลำดับ §87) · เดือนรับเงินก่อนเดือนริบ ⇒ ธง `[DEPOSIT-LATE-VAT]`
      "ภาษีถึงกำหนดงวด {เดือนรับเงิน} (รับเงิน dd/MM/yyyy) · นำส่งในงวด {เดือนริบ} (ใบกำกับภาษีของยอดที่ริบลงวันที่ … ย้อนไปเดือนรับเงินไม่ได้) · อาจมีเงินเพิ่ม §89/1 ·
      ห้ามนำส่งซ้ำ — ปรึกษานักบัญชี" ไม่ดูกำหนดยื่น/ล็อก · เดือนเดียวกัน = ไม่มีธง · `UndueReclassification` (ย้าย 21913→21911) = กติกา R2-1 เดิม
      (ย้อนเข้าเดือนรับเงินได้เพราะ JE ขาภาษีแยกลงวัน tax point)
    - **R3-2 ตัวล็อกยอดใบมัดจำตัวเดียว `LockDepositBalancesAsync`** (คีย์ `DepositRealizeKey` แบบไม่รอ → FOR UPDATE เรียงตาม Id → อ่านค่าล่าสุดของแถวที่ context ถือ ·
      ต้องอยู่ในธุรกรรม — ไม่มี = โยน ไม่ข้ามเงียบ) ถูกเรียกจาก: (ก) `RecognizeDepositOutputVatAsync` (ปุ่ม 🧾 VAT — เดิมชนกับริบแบบย้าย VAT พัก ⇒ Dr 21913 สองครั้ง) ·
      (ข) `AutoPostToJournalAsync` เส้นหักมัดจำแบบขับ JE หลายใบ + ใบเดียว (เดิม FOR UPDATE อย่างเดียว) · (ค) `VoidDocumentAsync` + `PurgeDocumentAsync` ล็อก
      **ทุกใบมัดจำที่จะคืนยอด** ก่อนขั้นแรก (`DepositIdsTouchedByAsync`: ใบนี้เอง · ตัดชำระ JV เข้าใบนี้ · รับรู้เพื่อใบนี้ · หักแบบขับ JE) · (ง) `PostCashSaleJournalAsync`
      (integration ขายเงินสด) **เปิดธุรกรรมเอง** (เดิมนอกธุรกรรม ⇒ `LoadTaxedDepositsByRefAsync` ข้ามล็อก) — ถอยธุรกรรมก่อนอ่านค่าจริงคืนให้ entity · และ
      `LoadTaxedDepositsByRefAsync(lockRows)` เรียกตัวเดียวกัน (ไม่มีทาง "ข้ามเมื่อไม่มีธุรกรรม" แล้ว) · เส้นเดิมที่ถือคีย์อยู่แล้ว: `RealizeDepositAsync` (session) ·
      `RefundDepositAsync` · `ApplyDepositToInvoiceAsync` ⇒ **ทุกเมธอดที่เขียน `DepositRealizedAmount`/`DepositRefundedAmount`/`DepositOutputVatRecognizedAt`/
      `DepositAppliedToDocumentId` ของใบมัดจำ (9 เมธอด · 37 จุดเขียน) อยู่ใต้คีย์เดียวกันผ่านจุดถือคีย์ 9 จุด** · ถูกถือ ⇒ `DEPOSIT-REALIZE-BUSY` "รอสักครู่" (ธุรกรรม rollback ทั้งก้อน — กดใหม่ได้) ·
      integration ขายเงินสดที่ชนล็อก ⇒ ตกเส้นเดิม "ตั้งหนี้แทน" พร้อมหมายเหตุ (เหมือนความล้มเหลวอื่นของ AutoPost)
    - **P-2** `LoadTaxedDepositsByRefAsync` ตัดสิน "ใบที่ถูกหักจริง" (`ResolveDeductedDeposits`) **ก่อน**ล็อก แล้วล็อกเฉพาะใบนั้น — เดิมล็อกทุกใบที่เลขอ้างอิงตรง
      ⇒ มัดจำค่าห้องกับเงินประกันใช้เลขจองเดียวกัน กำลังคืนเงินประกันอยู่ = เช็คเอาต์ล้ม "รอสักครู่"
    - **P-4 คืนยอดหักมัดจำแบบขับ JE หลายใบ** — `UnrealizeDrivesDepositAsync` แยกเลขด้วย `ResolveDrivesDepositIdsAsync` (ตัวเดียวกับเส้นหัก: `ParseDepositRefs` +
      เลขเราก่อน แล้ว Reference) · ฐานรายใบ `DepositReversalMath.SplitDrivesUnrealize` (ใบเดียว = ทุกขา 215/217 · หลายใบ = ผูกขาด้วยเลขใบในคำอธิบายที่เส้นหักเขียนเอง
      "มัดจำ {เลข}" ทั้งคำ · ผูกไม่ได้ ⇒ หมายเหตุ ⚠️ บนใบมัดจำ + log — ห้ามเดา) · เดิมเทียบ `DocumentNumber == DepositAppliedRef` ตรงตัว ⇒ "REC-9, REC-10" void แล้วยอด
      ไม่คืนเลย · purge ใบที่ void แล้ว (`skipAlreadyReleased`) คืนเฉพาะมัดจำที่ตัวชี้ยังชี้ใบนี้ (เดิมหัก `DepositRealizedAmount` ซ้ำ) · **ข้อมูลเดิม**: มัดจำหลายใบที่
      void ไปก่อนแก้ยังค้าง "รับรู้ครบ" — ไม่ migrate (ต้องแยกยอดจากคำอธิบาย JE · ให้เจ้าของตัดสิน) · purge ใบนั้นตอนนี้คืนให้ได้ (ตัวชี้ยังชี้ใบที่ void)
    - **P-1 อนุมัติล้มแล้วห้ามบันทึกของค้าง** — CMS `SyncBookingToErpAsync`: บันทึกก่อน → จุดตั้งต้น → อนุมัติล้ม ⇒ `TrackedChangeRevert` ถอย + reload
      (เดิม catch แล้ว `SaveChanges` ⇒ ใบ Approved ไม่มี JE) + หมายเหตุบนการจอง · ที่พักเช็คเอาต์: ใบร่างที่ค้างจากรอบที่อนุมัติล้ม (`LodgingCheckoutDraft.LeftoverOf`)
      ⇒ กดซ้ำ**ใช้ใบร่างเดิม** (`UpdateDocumentAsync` ด้วยเนื้อหารอบนี้ · ""/0 ล้างการหักมัดจำของรอบก่อน) · ชนิดเปลี่ยน/ค้างเกินหนึ่งใบ ⇒ ลบใบร่าง (ไม่มีเลขจริง — ไม่เกิดช่องว่าง §86/4)
...
- **Realize**: `RealizeDepositAsync(amount, revenueAccountCode, FinalInvoiceId?)` (`DocumentService.cs:3422` → ตัวลงบัญชีไม่ SaveChanges
  ...
  void/purge ใบสุดท้ายกลับได้ (`ReverseDepositRealizationsForAsync`) · audit-deposit P1-2 (realize หลังคืนบางส่วนของโหมดรอเรียกเก็บย้าย 21913 ทั้งก้อน)
  **ปิดแล้วรอบ 194 ทีม M** — ย้ายเท่าที่ยังพักจริงใน GL (`UndueVatToRecognize`)
...
- checker `tools/contact_taxid_only_match_check.py` (กติกา 1 query ผู้ติดต่อด้วยเลขภาษีอย่างเดียว · กติกา 2 `#soft` จับชื่อ/อีเมลหลัง `FindAsync` นอก SoftScope) ·
  baseline 3 จุด (Payroll ×2 · POS `#soft` 1 — OCR ×2 ย้ายมาใช้คีย์กลางแล้วรอบ 197) + `required_call_site_check` ล็อกทุกผู้เรียก ·
  เส้น OCR (สแกน/สร้างเอกสาร/หาใบต้นทาง) เดินตัวนี้ผ่าน `ContactTaxBranchKey.PickBranch` (`OcrVendorBranchContact`) และ `FindAsync`+`SoftScope` (ฝั่งผู้ซื้อ) — §1 OCR
...
| ปิดเงินประกัน (`POST …/security-deposit/settle` · `LodgingManage` · `SettleSecurityDepositAsync` · รอบ 194 · spec S3) | หลังเช็คเอาต์ (หรือยกเลิก/no-show) ตามลำดับ: ① **ตัดชำระยอดค้างของใบเช็คเอาต์** (`ApplyDepositToInvoiceAsync` — ค่าเสียหายตอนเช็คเอาต์เป็นรายการ folio ที่คิด VAT อยู่แล้ว ⇒ รับชำระหนี้ **ไม่ลดฐานภาษี** · ห้ามเข้า `DepositBaseDeducted`) → ② **ริบเป็นค่าเสียหาย** (`RealizeDepositAsync(ฐาน, ForfeitAs: Compensation)` — รายได้อื่นไม่มี VAT · บัญชี `DepositKind.ForfeitAccountCode` → 43080 → 43070 → ล้มดังพร้อมทางไปต่อ (รอบ 194 ทีม M P-c: เดิมตกไปค่าปรับยกเลิก/รายได้ค่าห้อง) · ต้องระบุรายการความเสียหาย) → ③ **คืนส่วนที่เหลือ** (`RefundDepositAsync` จากยอดจริงหลังขั้นก่อน · บัญชีที่เงินเข้า/ที่เลือก) หรือคงค้าง | แผน+ด่านทั้งหมดก่อนแตะบัญชี `LodgingDepositSettlement.PlanSecuritySettlement` (ตัดชำระต้องมีใบเช็คเอาต์ที่อนุมัติ · ลูกค้ารายเดียวกัน · ไม่เกินยอดค้าง · เงินประกันที่ออกใบกำกับแล้วตัดชำระไม่ได้ (VAT ซ้ำ) · รวมไม่เกินคงเหลือ) · ล็อกระดับการจองเดียวกับคืนเงิน · ห้ามลงวันที่ย้อนเข้างวด ภ.พ.30 ที่ยื่นแล้ว (เมื่อใบมี VAT) · ตัดชำระเพิ่ม `PaidAmount` · คงเหลือ 0 ⇒ ประทับ `SecurityDepositSettledAt` · ล้มกลางทาง = หมายเหตุ + audit + ข้อความบอกขั้นที่ทำแล้ว · หน้า front desk: บอกให้เลือก "หักเข้าใบที่คิด VAT" ถ้าเป็นค่าของที่ใช้ไป/ค่าบริการ |
...
**ตั้งค่าประเภทเงินมัดจำของที่พัก (รอบ 194 · `LodgingService.ApplyDepositKindsAsync`)**: `RoomDepositKindId` (ลักษณะ ราคา/นอกระบบ VAT) ·
`SecurityDepositKindId` (ลักษณะเงินประกัน) · `SecurityDepositAmount` (> 0 ต้องมีประเภท) · ประเภทต้องเป็นของบริษัทนี้ + ลักษณะถูกช่อง (ผิด ⇒ ข้อความไทย `DEPOSIT-KIND`) ·
...
**Invariants**: `Reservation.TotalAmount` = engine ณ วันจอง (snapshot ใน `PriceBreakdownJson`) · `FolioTotal` = Σ charges ที่ไม่ Cancelled · `PaidAmount` = มัดจำค่าห้อง + ยอดเก็บตอนเช็คเอาต์ + เงินประกันที่ตัดชำระใบเช็คเอาต์ − คืน (เงินประกันที่ยังถือไว้ไม่นับ) · `BalanceDue` = Total+Folio−Paid · เลขจอง `RES-{Code}-{yyMM}-{####}` ต่อที่พัก ภายใต้ `AdvisoryLockKey.For(cid,"lodging-res",propertyId)` · `PublicToken` 32 hex ต่อการจอง (unique index) · `GuestIdNumber` ไม่เคยออกจากเซิร์ฟเวอร์เต็ม (`MaskId`) · ทุก query มี `CompanyId` + ฝั่งสาธารณะเพิ่ม `SiteId`
...
_ก่อนหน้า: 2026-09-25 (รอบ 194 ทีม M3 หลังฝ่ายค้านรอบสาม: R3-1 tax point ใบกำกับของการริบ = วันที่ของใบ · R3-2 ตัวล็อกยอดใบมัดจำตัวเดียวทุกเส้น ·
P-1 อนุมัติล้มไม่บันทึกของค้าง/เช็คเอาต์ใช้ใบร่างเดิม · P-2 ล็อกเฉพาะใบที่ถูกหัก · P-4 คืนยอดหักมัดจำหลายใบ (§ มัดจำ "ทีม M3") — commit <pending>)_
