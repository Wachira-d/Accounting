...
balance + `node --check` → อัปเดต `DOCUMENT_FLOW`/`TEST_PLAN`/`CLAUDE.md` ในคอมมิตเดียวกัน

**G-02 ยังไม่ปิดโดยตั้งใจ** — ต้องเลือกก่อนว่าจะใช้ backend ไหน (S3 / Azure Blob /
shared volume) ซึ่งเป็นการตัดสินใจด้าน deployment ของเจ้าของโปรเจกต์ ไม่ใช่ของ agent
· การทำ interface แล้วเหลือ implementation เดียวที่ยังเขียน local disk = "merge
ครึ่ง ๆ กลาง ๆ" ที่ไฟล์นี้ห้ามไว้เอง
